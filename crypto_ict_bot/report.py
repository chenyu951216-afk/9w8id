...
def _eth_short_mode(analysis: dict[str, Any]) -> dict[str, Any]:
    trader = analysis.get("trader_mode")
...
def _eth_gate_reason(default_reason: str, state: str, gate_execution: dict[str, Any]) -> str:
    ...
    if action == "submitted":
        ...
        return f"ETH trader plan 已送出 Gate 限價單，entry {intent.get('entry_price')}，等待成交後切換持倉管理。"
    if action == "dry_run":
        return "ETH trader plan 可執行，但 Gate 目前是乾跑模式，沒有送出實單。"
    if action == "disabled":
        return "ETH trader plan 可執行，但 Gate 自動交易開關目前關閉，沒有送出實單。"
    if action == "blocked":
        ...
        return f"ETH trader plan 可執行，但 Gate live 安全條件未通過：{blocker_text}"
    if action == "error":
        return f"ETH trader plan 可執行，但 Gate 送單失敗：{gate_execution.get('message')}"
    return default_reason
...
def report_to_dict(report: SymbolReport) -> dict[str, Any]:
    ...
    eth_short_mode = _eth_short_mode(eth_analysis) if eth_analysis else {}
