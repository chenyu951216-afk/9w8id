...
def trade_action(report: SymbolReport) -> dict[str, Any]:
    ...
    if isinstance(stable, dict) and stable.get("code"):
        ...
        if stable_code in {"market", "limit", "watch", "avoid"}:
            ...
            return {
                "code": stable_code,
                "label": GATE_ACTION_LABELS.get(stable_code, raw["label"]),
...
def execution_plan(report: SymbolReport) -> dict[str, Any]:
    ...
    if should_execute:
        ...
        label = "待確認"
        mode = "不下單"
        summary = f"{direction} 到價但缺少確認：{action['reason']}"
...
def report_to_dict(report: SymbolReport) -> dict[str, Any]:
    ...
    signal_state = report.metadata.get("signal_state", {})
    candidate_grade = signal_state.get("priority_level") or report.metadata.get("candidate_grade") or report.grade
    candidate_status = signal_state.get("status") or report.metadata.get("candidate_status") or ""
    ...
    return complete_symbol_payload(report, payload, raw_action)
        ...
        for idx, report in enumerate(reports, start=1):
            ...
            writer.writerow(
                [
                    ...
                    signal_state.get("priority_level", report.metadata.get("candidate_grade", report.grade)),
                    signal_state.get("status", ""),
                    candidate_status_label(signal_state.get("status", "")),
