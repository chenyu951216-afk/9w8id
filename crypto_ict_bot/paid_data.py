...
def _configured_api_readiness(item: ExternalSymbolMetrics, keys: dict[str, Any]) -> dict[str, Any]:
    ...
    primary_paid_required = [provider_id for provider_id in ("coinglass",) if keys.get(provider_id)]
    optional_paid = [
        provider_id
        for provider_id in configured
        if provider_id not in {"coinglass", "exchange_public"}
    ]
    advisory_required = primary_paid_required
    ...
    advisory_missing = [
        provider_id
        for provider_id in advisory_required
        if status.get(provider_id, {}).get("state") not in {"read", "read_no_hit"}
    ]
    configured_failed = [
        provider_id
        for provider_id in [*execution_required, *primary_paid_required]
        if status.get(provider_id, {}).get("state") in {"failed", "pending"}
    ]
    optional_failed = [
        provider_id
        for provider_id in optional_paid
        if status.get(provider_id, {}).get("state") in {"failed", "pending"}
    ]
    return {
        "configured": configured,
        ...
        "primary_paid_required": primary_paid_required,
        "optional_paid_providers": optional_paid,
        ...
        "paid_policy": "CoinGlass is the only paid primary provider; absent optional paid APIs such as Coinalyze do not block execution.",
        "status": status,
    }
...
def _score_funding(item: ExternalSymbolMetrics, funding: float, source: str) -> None:
    ...
    if funding <= -0.0008:
        item.bonus_long += 1.2
        item.reasons_long.append(f"{source} extreme negative funding {funding:.5f}; shorts are crowded enough to create squeeze fuel.")
        item.warnings.append(f"{source} funding={funding:.5f}; short crowding is elevated, wait for sweep/retest confirmation.")
        return
    if funding >= 0.0008:
        item.bonus_short += 1.2
        item.reasons_short.append(f"{source} extreme positive funding {funding:.5f}; longs are crowded enough to create downside fuel.")
        item.warnings.append(f"{source} funding={funding:.5f}; long crowding is elevated, wait for sweep/retest confirmation.")
        return
    side = "long" if funding > 0 else "short"
    item.warnings.append(f"{source} funding={funding:.5f}; {side} participation is warm, not automatically invalid.")
    ...
    if isinstance(public, dict):
        ...
        if oi_change is not None and abs(oi_change) >= 8.0:
            ...
            if oi_change > 0 and flow_ratio >= 0.56 and "long_crowded" not in risk_flags:
                ...
                add("neutral", 0.0, f"OI +{oi_change:.1f}% shows participation, but taker flow is not directional enough.")
            elif oi_change < 0:
                add("neutral", 0.0, f"OI {oi_change:.1f}% shows deleveraging; continuation needs fresh flow.")
            if abs(oi_change) >= 30.0:
...
def _apply_funding_to_context(funding_value: Any, source: str, add: Any, flag: Any) -> None:
    ...
    if funding is None:
        return
    if funding >= 0.0008:
        add("short", min(5.0, 2.0 + funding / 0.0008), f"{source} extreme positive/crowded {funding:.5f}")
        flag("long_crowded")
    elif funding >= 0.00015:
        add("neutral", 0.0, f"{source} positive funding {funding:.5f}; trend participation is warm.")
    elif funding <= -0.0008:
        add("long", min(5.0, 2.0 + abs(funding) / 0.0008), f"{source} extreme negative/crowded {funding:.5f}")
        flag("short_crowded")
    elif funding <= -0.00015:
        add("neutral", 0.0, f"{source} negative funding {funding:.5f}; short participation is warm.")
