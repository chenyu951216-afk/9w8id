...
def enrich_reports(reports: list[SymbolReport], config: dict[str, Any]) -> dict[str, Any]:
    ...
    public_failure = 0
    public_error = ""
    public_reports = reports[: min(len(reports), 70)]
    public_timeout = max(20.0, min(75.0, float(paid_cfg.get("public_derivatives_timeout_seconds", 45))))
...
def _apply_external(score: DirectionScore, item: ExternalSymbolMetrics, direction: str) -> None:
    ...
    if "paid_data" not in score.feature_max_scores:
        score.bonus_max_score += 8.0
        score.feature_max_scores["paid_data"] = 8.0
    score.score += bonus
