...
from .gate_executor import attach_gate_execution, check_gate_connection, fetch_gate_ticker, manage_gate_position
...
class DashboardState:
    def __init__(self) -> None:
        ...
        self.gate_price: dict[str, Any] = {}
        self.gate_position: dict[str, Any] = {}
        self.gate_management: dict[str, Any] = {}
        self.gate_price_running = False
        self.last_gate_price_ts = 0.0
        self.gate_price_thread: threading.Thread | None = None
        self.gate_monitor_running = False
        self.last_gate_monitor_ts = 0.0
        self.gate_monitor_thread: threading.Thread | None = None
    ...
    def snapshot(self, since: int | None = None) -> dict[str, Any]:
        self.start_due_scan()
        self.start_gate_price_due()
        self.start_gate_monitor_due()
        with self.lock:
    ...
    def _runtime_payload_locked(self) -> dict[str, Any]:
        return {
            ...
            "gate_price": self.gate_price,
            "gate_price_running": self.gate_price_running,
            "last_gate_price_ts": self.last_gate_price_ts,
            "gate_position": self.gate_position,
            "gate_management": self.gate_management,
            "gate_monitor_running": self.gate_monitor_running,
            "last_gate_monitor_ts": self.last_gate_monitor_ts,
        }
    ...
    def start_gate_price_due(self) -> bool:
        with self.lock:
            gate = self.config.get("gate_trading", {}) if isinstance(self.config.get("gate_trading"), dict) else {}
            if self.gate_price_running:
                return False
            poll_seconds = max(1, min(15, int(gate.get("price_poll_seconds") or 1)))
            if time.time() - self.last_gate_price_ts < poll_seconds:
                return False
            config = self.config
            self.gate_price_running = True
            self.gate_price_thread = threading.Thread(
                target=self._gate_price_worker,
                args=(config,),
                daemon=True,
            )
            self.gate_price_thread.start()
            return True

    def _gate_price_worker(self, config: dict[str, Any]) -> None:
        try:
            price = fetch_gate_ticker(config)
        except Exception as exc:
            price = {
                "version": "gate_eth_executor_error",
                "ok": False,
                "errors": [f"Gate ticker failed: {type(exc).__name__}: {exc}"],
                "updated_at": datetime.now(timezone.utc).isoformat(),
            }
        with self.lock:
            self.gate_price = price if isinstance(price, dict) else {}
            self.gate_price_running = False
            self.last_gate_price_ts = time.time()

    def start_gate_monitor_due(self) -> bool:
        with self.lock:
            gate = self.config.get("gate_trading", {}) if isinstance(self.config.get("gate_trading"), dict) else {}
            if not gate.get("position_management_enabled", True):
                return False
            if self.gate_monitor_running:
                return False
            poll_seconds = max(1, min(30, int(gate.get("position_poll_seconds") or 2)))
            if time.time() - self.last_gate_monitor_ts < poll_seconds:
                return False
            config = self.config
            eth_meta = self.meta.get("eth_analysis") if isinstance(self.meta.get("eth_analysis"), dict) else {}
            eth_analysis = eth_meta.get("analysis") if isinstance(eth_meta.get("analysis"), dict) else eth_meta
            self.gate_monitor_running = True
            self.gate_monitor_thread = threading.Thread(
                target=self._gate_monitor_worker,
                args=(config, eth_analysis),
                daemon=True,
            )
            self.gate_monitor_thread.start()
            return True

    def _gate_monitor_worker(self, config: dict[str, Any], eth_analysis: dict[str, Any]) -> None:
        try:
            management = manage_gate_position(config, eth_analysis)
            position = management.get("position", {}) if isinstance(management, dict) else {}
        except Exception as exc:
            management = {
                "version": "gate_eth_executor_error",
                "errors": [f"Gate monitor failed: {type(exc).__name__}: {exc}"],
                "updated_at": datetime.now(timezone.utc).isoformat(),
            }
            position = {}
        with self.lock:
            self.gate_position = position if isinstance(position, dict) else {}
            self.gate_management = management if isinstance(management, dict) else {}
            self.gate_monitor_running = False
            self.last_gate_monitor_ts = time.time()
...
def scheduler_loop(state: DashboardState) -> None:
    while True:
        ...
        state.start_gate_price_due()
        state.start_gate_monitor_due()
