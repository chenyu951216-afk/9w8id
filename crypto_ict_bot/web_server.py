...
from .report import report_to_dict, selected_side, write_csv, write_html, write_json
    ...
    def snapshot(self) -> dict[str, Any]:
        with self.lock:
            ...
            for idx, report in enumerate(self.reports, start=1):
                ...
                item["selected_warnings"] = selected_side(report).warnings
                reports.append(item)
            return {
    ...
    def _scan_worker(self, reason: str) -> None:
        ...
        try:
            ...
            with self.lock:
                ...
                self.provider_status = provider_statuses(config)
...
def scheduler_loop(state: DashboardState) -> None:
    while True:
        time.sleep(2)
        with state.lock:
            due = time.time() >= state.next_refresh_ts
            running = state.running
        if due and not running:
            state.start_scan("15 分鐘自動刷新")
