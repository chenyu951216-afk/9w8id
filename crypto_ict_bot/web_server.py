...
def _report_sort_key(report: Any) -> tuple[int, float, float]:
    priority = report.metadata.get("signal_state", {}).get("priority_level", report.metadata.get("candidate_grade", ""))
    priority_rank = {"A": 5, "B": 4, "C": 3, "D": 2, "X": 1}.get(str(priority), 0)
    return priority_rank, float(report.score or 0.0), float(report.quote_volume_24h or 0.0)
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _serve_file(self, path: Path) -> None:
        ...
        if str(path).startswith("reports") or path.suffix in {".html", ".json", ".csv"}:
