...
def _executable_payloads(reports: list[SymbolReport]) -> list[dict[str, Any]]:
    ...
    for report in reports:
        ...
        if payload.get("should_execute") is True or payload.get("eth_should_notify") is True:
...
def _discord_message(
    ...
    if payload.get("eth_analysis"):
        return _eth_discord_message(payload, meta, file_name, username)
...
def _eth_discord_message(
    payload: dict[str, Any],
    meta: dict[str, Any],
    file_name: str,
    username: str,
) -> dict[str, Any]:
    symbol = str(payload.get("symbol") or "ETHUSDT")
    eth = payload.get("eth_analysis") if isinstance(payload.get("eth_analysis"), dict) else {}
    short_mode = eth.get("modes", {}).get("short_term", {}) if isinstance(eth.get("modes"), dict) else {}
    swing_mode = eth.get("modes", {}).get("swing", {}) if isinstance(eth.get("modes"), dict) else {}
    gate = payload.get("gate_execution") if isinstance(payload.get("gate_execution"), dict) else {}
    direction = str(eth.get("direction_label") or payload.get("direction_label") or "-")
    primary_state = str(eth.get("primary_plan_state") or "-")
    lifecycle = str(eth.get("plan_lifecycle") or "-")
    session = eth.get("session", {}) if isinstance(eth.get("session"), dict) else {}
    entry = _entry_text(short_mode.get("entry_zone") or payload.get("entry_zone"))
    stop = fmt_price(_as_float(short_mode.get("stop") or payload.get("stop")))
    rr = short_mode.get("rr", payload.get("rr"))
    rr_text = "-" if rr is None else f"{float(rr):.2f}R"
    targets = _tp_text({"TP1": _tp_at(short_mode, 0), "TP2": _tp_at(short_mode, 1), "TP3": _tp_at(short_mode, 2)})
    blockers = _list_text(short_mode.get("hard_blockers") or payload.get("hard_blockers") or [], limit=4)
    notes = _list_text(short_mode.get("soft_notes") or eth.get("market_psychology") or [], limit=4)
    gate_action = str(gate.get("action") or "not_configured")
    gate_message = str(gate.get("message") or "-")
    description = str(short_mode.get("decision") or payload.get("execution_summary") or "ETH dedicated strategy update.")
    return {
        "username": username,
        "content": f"ETH dedicated strategy update: **{symbol}** `{primary_state}`. Full report attached as `{file_name}`.",
        "allowed_mentions": {"parse": []},
        "embeds": [
            {
                "title": f"{symbol} ETH dedicated plan",
                "description": description[:900],
                "color": 0x2ECC71 if str(eth.get("direction")) == "long" else 0xE74C3C if str(eth.get("direction")) == "short" else 0x95A5A6,
                "timestamp": str(meta.get("generated_at") or payload.get("data_time") or ""),
                "fields": [
                    {"name": "Direction", "value": direction, "inline": True},
                    {"name": "Plan State", "value": f"{primary_state} / {lifecycle}", "inline": True},
                    {"name": "Session", "value": f"{session.get('name', '-')} / {session.get('bias', '-')}", "inline": False},
                    {"name": "Short Score", "value": f"{float(short_mode.get('quality_score') or 0):.1f}", "inline": True},
                    {"name": "Swing Score", "value": f"{float(swing_mode.get('quality_score') or 0):.1f}", "inline": True},
                    {"name": "Gate", "value": _gate_field_text(gate_action, gate_message, gate), "inline": False},
                    {"name": "Entry", "value": entry, "inline": True},
                    {"name": "Stop", "value": stop, "inline": True},
                    {"name": "RR", "value": rr_text, "inline": True},
                    {"name": "Targets", "value": targets, "inline": False},
...
def _gate_field_text(action: str, message: str, gate: dict[str, Any]) -> str:
    lines = [f"{action}: {message}"]
    blockers = gate.get("live_blockers") if isinstance(gate.get("live_blockers"), list) else []
    entry_guard = gate.get("entry_guard") if isinstance(gate.get("entry_guard"), dict) else {}
    guard_blockers = entry_guard.get("blockers") if isinstance(entry_guard.get("blockers"), list) else []
    all_blockers = []
    for item in list(blockers) + list(guard_blockers):
        text = str(item or "").strip()
        if text and text not in all_blockers:
            all_blockers.append(text)
    if all_blockers:
        lines.append("Live blockers:")
        lines.extend(f"- {item}" for item in all_blockers[:5])
    return "\n".join(lines)[:1000]
...
def _alert_signature(payload: dict[str, Any]) -> str:
    ...
    if eth:
        ...
        signature = {
            ...
            "direction": eth.get("direction"),
            "plan_state": eth.get("primary_plan_state"),
            "lifecycle": eth.get("plan_lifecycle"),
            "plan_id": short_mode.get("plan_id"),
            "entry_zone": short_mode.get("entry_zone"),
            "stop": short_mode.get("stop"),
            "rr": short_mode.get("rr"),
        }
