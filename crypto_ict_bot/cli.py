...
from .gate_executor import attach_gate_execution
...
def scan(args: argparse.Namespace) -> int:
    config = load_config()
    if config.get("eth", {}).get("enabled", True):
        args.symbols = config["eth"].get("symbol", "ETHUSDT")
        args.top = 1
        args.min_volume = 0.0
        args.workers = 1
    try:
    ...
    meta = {
        ...
        "eth_analysis": eth_meta,
        "gate_trading": gate_meta,
    }
...
def _report_sort_key(report: object) -> tuple[int, float, float, float]:
    return opportunity_sort_key(report)  # type: ignore[arg-type]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan_parser.add_argument("--top", type=int, default=1, help="top symbols by 24h quote volume")
    scan_parser.add_argument("--symbols", default="ETHUSDT", help="ETH dedicated symbol, defaults to ETHUSDT")
    scan_parser.add_argument("--min-volume", type=float, default=0.0, help="minimum 24h quote volume in USDT")
