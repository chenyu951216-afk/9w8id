...
const HEARTBEAT_RUNNING_MS = 1000;
const GATE_LIVE_CONFIRMATION = "I_UNDERSTAND_GATE_200X_ETH_RISK";
...
function gateStatusText(item, ethShort = {}) {
  const gate = item.gate_execution || {};
  const action = gate.action || "disabled";
  const blockers = Array.isArray(gate.live_blockers) ? gate.live_blockers : [];
  const onlyWaitingStrategy = blockers.length === 1 && blockers[0].includes("尚未進入 execute_ready");
  if (ethShort.state && ethShort.state !== "execute_ready" && onlyWaitingStrategy) return "Gate ready / 策略等待入場";
  if (action === "submitted") return "Gate 已送單";
  if (action === "dry_run") return "Gate 乾跑";
  if (action === "disabled") return "Gate 關閉";
  if (action === "error") return "Gate 送單錯誤";
  if (action === "blocked") return onlyWaitingStrategy ? "Gate ready / 策略等待入場" : "Gate 安全阻擋";
  return `Gate ${action}`;
}

function gateSafetyText(gateExecution = {}) {
  const blockers = Array.isArray(gateExecution.live_blockers) ? gateExecution.live_blockers : [];
  if (blockers.length === 1 && blockers[0].includes("尚未進入 execute_ready")) {
    return "Gate 已就緒，策略尚未進入 execute_ready，所以不送單。";
  }
  return blockers.join(" / ") || "dry-run or ready";
}

function ethTraderMode(item = {}) {
  const analysis = item.eth_analysis || {};
  return analysis.trader_mode || analysis.modes?.short_term || analysis.modes?.swing || {};
}
...
function renderSettings(config) {
  ...
  if ($("gateManagementEnabled")) $("gateManagementEnabled").checked = gate.position_management_enabled !== false;
  if ($("gateBreakEvenEnabled")) $("gateBreakEvenEnabled").checked = gate.break_even_enabled !== false;
  if ($("gateTrailingEnabled")) $("gateTrailingEnabled").checked = gate.trailing_stop_enabled !== false;
  if ($("gateCancelBotOrdersWhenFlat")) $("gateCancelBotOrdersWhenFlat").checked = gate.cancel_bot_orders_when_flat !== false;
  ...
  if ($("gatePricePollSeconds")) $("gatePricePollSeconds").value = gate.price_poll_seconds || 1;
  if ($("gatePollSeconds")) $("gatePollSeconds").value = gate.position_poll_seconds || 2;
  if ($("gateBreakEvenTrigger")) $("gateBreakEvenTrigger").value = gate.break_even_trigger_r ?? 1;
  if ($("gateBreakEvenLock")) $("gateBreakEvenLock").value = gate.break_even_lock_r ?? 0.05;
  if ($("gateTrailingTrigger")) $("gateTrailingTrigger").value = gate.trailing_trigger_r ?? 1.8;
  if ($("gateTrailingDistance")) $("gateTrailingDistance").value = gate.trailing_distance_r ?? 0.75;
  ...
  if ($("gateLiveConfirm")) $("gateLiveConfirm").placeholder = gate.confirm_live_trading?.configured ? gate.confirm_live_trading.masked : GATE_LIVE_CONFIRMATION;
  syncGateLiveControls();
}

function syncGateLiveControls() {
  const liveMode = Boolean($("gateEnabled")?.checked) && $("gateDryRun")?.checked === false;
  if ($("gatePlaceExits") && liveMode) {
    $("gatePlaceExits").checked = true;
    $("gatePlaceExits").disabled = true;
  } else if ($("gatePlaceExits")) {
    $("gatePlaceExits").disabled = false;
  }
}
...
function renderMetrics(data) {
  ...
  $("lastUpdated").textContent = data.last_completed
    ...
    : `尚未完成首次刷新${errorText}`;
  renderGatePosition(data.gate_position || {}, data.gate_management || {}, data.gate_price || {});
...
function renderGatePosition(positionPayload = {}, management = {}, pricePayload = {}) {
  const summary = $("gatePositionSummary");
  const meta = $("gatePositionMeta");
  const actionsNode = $("gateManagementActions");
  const ordersNode = $("gateBotOrders");
  if (!summary || !meta || !actionsNode || !ordersNode) return;
  const pos = positionPayload.position || {};
  const active = management.state?.active_position || {};
  const latest = chooseLatestGateTicker(pricePayload, management.latest_ticker || {});
  const apiOk = Boolean(positionPayload.api_configured);
  const positionOk = Boolean(positionPayload.position_ok);
  const hasPosition = Boolean(positionPayload.has_position || pos.has_position);
  const latestMark = pos.mark_price ?? latest.mark_price;
  const priceAge = latest.updated_at ? ` · 價格 ${new Date(latest.updated_at).toLocaleTimeString("zh-TW")}` : "";
  const modeText = management.live_ready ? "live" : (management.dry_run === false ? "blocked" : "dry-run");
  meta.textContent = management.updated_at
    ? `持倉 ${new Date(management.updated_at).toLocaleString("zh-TW")} / ${modeText}${priceAge}`
    : (latest.updated_at ? `價格 ${new Date(latest.updated_at).toLocaleString("zh-TW")} / 等待持倉管理` : "等待 Gate 追蹤");
  const cards = [
    ["Gate API", apiOk ? (positionOk ? "已連線" : "待確認") : "未設定"],
    ["ETH 即時價", `${fmtPrice(latest.mark_price ?? pos.mark_price)} / ${fmtPrice(latest.last_price)}`],
    ["持倉狀態", hasPosition ? `${directionLabel(pos.direction)} ${pos.contracts_abs ?? "-"} 張` : "空倉"],
    ["ETH 數量", pos.quantity_eth ?? "-"],
    ["進場 / 標記", `${fmtPrice(pos.entry_price)} / ${fmtPrice(latestMark)}`],
    ["未實現 PnL", pos.unrealised_pnl === null || pos.unrealised_pnl === undefined ? "-" : `${Number(pos.unrealised_pnl).toFixed(4)} USDT`],
    ["R 倍數", active.last_r_multiple === null || active.last_r_multiple === undefined ? "-" : `${Number(active.last_r_multiple).toFixed(3)}R`],
    ["初始 / 目前止損", `${fmtPrice(active.initial_stop)} / ${fmtPrice(active.last_stop)}`],
    ["保護單", `${(management.bot_price_orders || []).length} 筆 t-eth-*`],
    ["價格追蹤", latest.poll_seconds ? `${latest.poll_seconds}s` : "-"],
    ["持倉管理", management.poll_seconds ? `${management.poll_seconds}s` : "-"],
    ["管理模式", management.management_enabled === false ? "關閉" : modeText],
    ["強平價", fmtPrice(pos.liq_price)],
    ["保證金", pos.margin_usdt === null || pos.margin_usdt === undefined ? "-" : `${Number(pos.margin_usdt).toFixed(4)} USDT`],
  ];
  summary.innerHTML = cards.map(([label, value]) => `<article class="stat-card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></article>`).join("");

  const actions = [
    ...(management.actions || []),
    ...(management.planned_actions || []),
    ...(management.errors || []).map((error) => ({ type: "error", message: error })),
  ];
  actionsNode.innerHTML = "";
  (actions.length ? actions : [{ type: "idle", message: hasPosition ? "持倉監控中，尚未需要調整。" : "目前空倉，等待策略或 Gate 訊號。" }]).forEach((action) => {
    const li = document.createElement("li");
    const bits = [
      action.type,
      action.reason ? `reason ${action.reason}` : "",
      action.r_multiple !== undefined ? `${Number(action.r_multiple).toFixed(3)}R` : "",
      action.old_stop !== undefined && action.new_stop !== undefined ? `${fmtPrice(action.old_stop)} → ${fmtPrice(action.new_stop)}` : "",
      action.price !== undefined ? `price ${fmtPrice(action.price)}` : "",
  ...
  const orders = management.bot_price_orders || [];
  (orders.length ? orders : [{ text: "-", trigger_price: null, close_size: null, status: "無機器人條件單" }]).forEach((order) => {
    const li = document.createElement("li");
    li.textContent = `${order.text || "-"} / ${order.status || "-"} / size ${order.close_size ?? "-"} / trigger ${fmtPrice(order.trigger_price)}`;
    ordersNode.appendChild(li);
  });
}

function chooseLatestGateTicker(primary = {}, fallback = {}) {
  const primaryTs = primary.updated_at ? Date.parse(primary.updated_at) || 0 : 0;
  const fallbackTs = fallback.updated_at ? Date.parse(fallback.updated_at) || 0 : 0;
  if (primaryTs >= fallbackTs && (primary.ok || primary.updated_at)) return primary;
  return fallback || {};
}
...
function baseRowHtml(item) {
  const ethShort = ethTraderMode(item);
...
function showDetail(item) {
  ...
  $("detailMeta").textContent = ethShort.state
    ? `ETH ${item.eth_analysis?.direction_label || item.direction || "-"} · 交易員 ${ethShort.state} · ${ethShort.decision || "-"} · RR ${Number(ethShort.rr || 0).toFixed(2)} · ${gateStatusText(item, ethShort)}`
...
function renderDiagnostics(item) {
  ...
  const ethShort = eth.trader_mode || eth.modes?.short_term || {};
  if (Object.keys(eth).length) {
    values.unshift(
      ...
      `ETH trader: score ${fmtMaybe(ethShort.quality_score)} / min ${fmtMaybe(ethShort.min_score)} | state ${ethShort.state || "-"} | RR ${fmtMaybe(ethShort.rr)} | entry distance ${fmtMaybe(ethShort.entry_distance_pct, 4)}% | source ${ethShort.source_mode || "-"}`
    );
...
function renderExecutionPlan(item) {
  ...
  const values = [
    `ETH trader state：${ethShort.state || "-"} · ${ethShort.decision || "-"}`,
    `Gate：${gateStatusText(item, ethShort)} · ${gateExecution.message || "-"}`,
    ...
    `Gate safety：${gateSafetyText(gateExecution)}`,
...
function renderRiskPlan(item) {
  ...
  const eth = item.eth_analysis || {};
  const ethShort = eth.trader_mode || eth.modes?.short_term || {};
  if (ethShort.entry_zone) values.push(`ETH 交易員入場區：${fmtPrice(ethShort.entry_zone[0])} - ${fmtPrice(ethShort.entry_zone[1])}`);
  if (ethShort.stop !== null && ethShort.stop !== undefined) values.push(`ETH 交易員止損：${fmtPrice(ethShort.stop)}`);
...
async function saveSettings() {
  ...
  const gateEnabled = $("gateEnabled")?.checked || false;
  const gateDryRun = $("gateDryRun")?.checked !== false;
  const gateLiveMode = gateEnabled && !gateDryRun;
  await api("/api/settings", {
    ...
    body: JSON.stringify({
      ...
      gate_trading: {
        ...
        place_exit_orders: gateLiveMode ? true : ($("gatePlaceExits")?.checked || false),
        exit_order_expiration_seconds: Number($("gateExitExpiration")?.value || 86400),
        position_management_enabled: $("gateManagementEnabled")?.checked !== false,
        price_poll_seconds: Number($("gatePricePollSeconds")?.value || 1),
        position_poll_seconds: Number($("gatePollSeconds")?.value || 2),
        break_even_enabled: $("gateBreakEvenEnabled")?.checked !== false,
        break_even_trigger_r: Number($("gateBreakEvenTrigger")?.value || 1),
        break_even_lock_r: Number($("gateBreakEvenLock")?.value || 0.05),
        trailing_stop_enabled: $("gateTrailingEnabled")?.checked !== false,
        trailing_trigger_r: Number($("gateTrailingTrigger")?.value || 1.8),
        trailing_distance_r: Number($("gateTrailingDistance")?.value || 0.75),
        cancel_bot_orders_when_flat: $("gateCancelBotOrdersWhenFlat")?.checked !== false,
...
function renderGateStatus(gate) {
  ...
  const safety = (gate.safety_blockers || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("");
  ...
  node.innerHTML = `
    <article class="api-card ${gate.live_ready ? "api-ready" : "api-missing"}">
      ...
      ${safety ? `<p>Live safety blockers</p><ul>${safety}</ul>` : ""}
...
if ($("fillGateLiveConfirm")) {
  $("fillGateLiveConfirm").addEventListener("click", () => {
    $("gateLiveConfirm").value = GATE_LIVE_CONFIRMATION;
  });
}
if ($("gateEnabled")) $("gateEnabled").addEventListener("change", syncGateLiveControls);
if ($("gateDryRun")) $("gateDryRun").addEventListener("change", syncGateLiveControls);
