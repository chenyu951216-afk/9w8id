...
function showDetail(item) {
  ...
  $("detailMeta").textContent = `${item.trade_action_label || "未分類"} · ${item.trade_action_reason || ""} · 候選 ${item.candidate_grade || "-"} / ${statusLabel(item.candidate_status || item.lifecycle_state || "-")} · Setup ${Number(item.setup_score || 0).toFixed(1)} · Execution ${Number(item.execution_score || 0).toFixed(1)} · Conviction ${Number(item.direction_conviction || 0).toFixed(1)} · Expected R ${Number(item.expected_R || 0).toFixed(2)} · 原始 ${Number(item.score || 0).toFixed(1)} · 共振 ${Number(model.bonus_score || item.bonus_score || 0).toFixed(1)}`;
  renderRiskPlan(item);
