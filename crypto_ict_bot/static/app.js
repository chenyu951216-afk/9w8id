...
async function loadState() {
  ...
  try {
    const data = await api("/api/state", { timeoutMs: 12000 });
...
async function loadHeartbeat() {
  ...
  try {
    ...
    if (state.stateVersion !== null && version !== state.stateVersion && !state.loadingState) {
      await loadState();
    } else if (state.stateVersion === null && !state.loadingState) {
      await loadState();
    }
  } catch (error) {
...
function renderRows() {
  ...
  rows.forEach((item) => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge ${executionClass(item.execution_label)}">${item.execution_label || "-"}</span></td>
      <td class="plan-cell">${item.execution_summary || "-"}</td>
...
function showDetail(item) {
  ...
  $("detailMeta").textContent = `${item.trade_action_label || "未分類"} · ${item.trade_action_reason || ""} · 校準分 ${Number(model.calibrated_score || item.score || 0).toFixed(1)} · 原始核心 ${Number(model.core_score || item.core_score || 0).toFixed(1)} · 共振 ${Number(model.bonus_score || item.bonus_score || 0).toFixed(1)} · 資料完整度 ${Number(item.data_completeness || 0).toFixed(0)}%`;
...
function renderDiagnostics(item) {
  ...
  const values = [
    ...
    `最終校準分：${Number(model.calibrated_score || item.score || 0).toFixed(1)} / 100`,
...
function renderSignalState(item) {
  ...
  const values = [
    `穩定動作：${s.stable_action?.label || item.trade_action_label || "-"}`,
    `原始模型：${item.raw_trade_action_label || "-"}`,
    `平滑分數：${Number(s.smoothed_score || item.score || 0).toFixed(1)}`,
    `連續確認：${Number(s.confirm_count || 0)} 次`,
    `失效計數：${Number(s.fail_count || 0)} 次`,
  ];
...
setInterval(loadHeartbeat, 2000);
