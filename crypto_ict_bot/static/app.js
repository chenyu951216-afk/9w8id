...
function renderSettings(config) {
  ...
  if ($("gatePollSeconds")) $("gatePollSeconds").value = gate.position_poll_seconds || 10;
...
function renderMetrics(data) {
  ...
  $("lastUpdated").textContent = data.last_completed
    ...
    : `尚未完成首次刷新${errorText}`;
  renderGatePosition(data.gate_position || {}, data.gate_management || {});
...
function renderGatePosition(positionPayload = {}, management = {}) {
  ...
  meta.textContent = management.updated_at
    ? `最後輪詢 ${new Date(management.updated_at).toLocaleString("zh-TW")} / ${modeText}`
    : "等待 Gate 持倉輪詢";
  const cards = [
    ...
    ["進場 / 標記", `${fmtPrice(pos.entry_price)} / ${fmtPrice(pos.mark_price)}`],
    ...
    ["輪詢", management.poll_seconds ? `${management.poll_seconds}s` : "-"],
  ...
  await api("/api/settings", {
    ...
    body: JSON.stringify({
      ...
      gate_trading: {
        ...
        position_poll_seconds: Number($("gatePollSeconds")?.value || 10),
