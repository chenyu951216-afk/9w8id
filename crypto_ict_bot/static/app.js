...
function showDetail(item) {
  ...
  $("detailMeta").textContent = `${item.trade_action_label || "未分類"} · ${item.trade_action_reason || ""} · 核心 ${Number(model.core_score || item.core_score || 0).toFixed(1)} 分 + 共振 ${Number(model.bonus_score || item.bonus_score || 0).toFixed(1)} 分 · 資料完整度 ${Number(item.data_completeness || 0).toFixed(0)}%`;
...
function renderDiagnostics(item) {
  ...
  const model = item.score_model || {};
  const skippedOptional = Object.keys(model.skipped_optional_not_penalized || {});
  ...
  if (skippedOptional.length) values.push(`未觸發加分項：${skippedOptional.map(featureLabel).join("、")}，已排除分母。`);
