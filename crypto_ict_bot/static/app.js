...
function renderRows() {
  ...
  rows.forEach((item) => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge ${gradeClass(item.candidate_grade)}">${item.candidate_grade || item.grade || "-"}</span></td>
      <td>${statusLabel(item.candidate_status || item.signal_state?.status || "-")}</td>
      <td>${trendLabel(item.score_trend || item.signal_state?.score_trend || "-")}</td>
