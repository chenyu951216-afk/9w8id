...
function renderProviders(providers, config) {
  ...
  providers.forEach((provider) => {
    ...
    const badgeClass = configured ? "api" : "off";
    card.innerHTML = `
      <header>
        <div>
...
function showDetail(item) {
  ...
  $("detailMeta").textContent = item.passed ? "已達及格線" : "未達及格線，仍保留在榜單";
  ...
  Object.entries(item.feature_scores || {}).forEach(([name, value]) => {
    ...
    const pct = Math.max(0, Math.min(100, Number(value) / 14 * 100));
    row.innerHTML = `<span>${featureLabel(name)}</span><div><i style="width:${pct}%"></i></div><b>${Number(value).toFixed(1)}</b>`;
