...
function showDetail(item) {
  ...
  $("detailMeta").textContent = `${item.passed ? "已達及格線" : "未達及格線，仍保留在榜單"} · 資料完整度 ${Number(item.data_completeness || 0).toFixed(0)}% · 可用分母 ${Number(item.available_score_max || 0).toFixed(1)}`;
