...
def _zone_distance_abs(price: float, lower: float, upper: float) -> float:
    if lower <= price <= upper:
        return 0.0
    return min(abs(price - lower), abs(price - upper))


def recent_relevant_fvg(
    candles: list[Candle],
    direction: str,
    max_age: int = 90,
    ...
    latest_atr = atr(candles, 14) or median_range(candles, 30) or price * 0.001
    candidates = [gap for gap in detect_fvgs(candles, max_age=max_age) if gap.direction == direction and not gap.filled]
    if anchor_index is not None:
        candidates = [gap for gap in candidates if gap.index >= anchor_index - 2 or abs(gap.index - anchor_index) <= 6]
    candidates = [
        gap
        for gap in candidates
        if _zone_distance_abs(price, gap.lower, gap.upper) <= latest_atr * max_distance_atr
    ]
    if not candidates:
        return None

    def rank(gap: FVG) -> tuple[float, int]:
        anchor_distance = abs(gap.index - anchor_index) if anchor_index is not None else 0
        distance_atr = _zone_distance_abs(price, gap.lower, gap.upper) / max(latest_atr, 1e-12)
        return (anchor_distance + distance_atr, -gap.index)
...
def ote_zone(candles: list[Candle], direction: str, lookback: int = 90) -> tuple[float, float, float] | None:
    ...
    if len(sample) < 20:
        return None
    sweep = detect_liquidity_sweep(sample, direction, lookback=len(sample))
    if not sweep:
        return None
    structure = detect_structure_break(sample, direction, after_index=sweep.index, lookback=max(12, len(sample) - sweep.index))
    if not structure:
        return None
    displacement = detect_displacement(
        sample,
        direction,
        after_index=structure.index,
        lookback=max(12, min(45, len(sample) - structure.index + 4)),
    )

    impulse_end = displacement.index if displacement else structure.index
    leg_start = max(0, sweep.index - 2)
    leg_tail = sample[structure.index : min(len(sample), max(impulse_end + 4, structure.index + 3))]
    if not leg_tail:
        return None

    price = candles[-1].close
    if direction == "long":
        low = min(c.low for c in sample[leg_start : sweep.index + 1])
        high = max(c.high for c in leg_tail)
        if high <= low:
            return None
        ...
        high = max(c.high for c in sample[leg_start : sweep.index + 1])
        low = min(c.low for c in leg_tail)
        if high <= low:
            return None
