...
ENV_SETTING_MAP = {
    ...
    ("gate_trading", "margin_mode"): ("GATE_MARGIN_MODE", str),
...
def normalize_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    gate["margin_mode"] = str(gate.get("margin_mode") or "cross").lower()
    if gate["margin_mode"] not in {"cross", "isolated"}:
        gate["margin_mode"] = "cross"
    ...
    gate["place_exit_orders"] = bool(gate.get("place_exit_orders", True))
    if gate["enabled"] and not gate["dry_run"]:
        gate["margin_mode"] = "cross"
        gate["set_leverage_before_order"] = True
        gate["place_exit_orders"] = True
        gate["position_management_enabled"] = True
