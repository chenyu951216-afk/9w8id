...
def _add(score: DirectionScore, feature: str, points: float, reason: str) -> None:
...
def _risk_setup(
    ...
    if not candles:
        ...
        if direction == "long":
            ...
            stop = entry + buffer
            target = entry - buffer * 2
        return (zone_low, zone_high), stop, target, 2.0
    ...
    if direction == "long":
        ...
        target = buy_side if buy_side and buy_side > entry else entry + risk * 2.0
        ...
        target = sell_side if sell_side and sell_side < entry else entry - risk * 2.0
        reward = max(entry - target, 0.0)
    rr = reward / max(risk, 1e-12)
    return (zone_low, zone_high), stop, target, rr
...
def _evaluate_direction(
    ...
    score = DirectionScore(direction=direction, max_score=sum(WEIGHTS.values()))
    ...
    if sweep:
        ...
        points = WEIGHTS["liquidity_sweep"] * min(1.0, 0.55 + sweep.strength / 2.0)
    ...
    if structure:
        base = 12.0 if structure.kind == "MSS" else 8.0
        if ltf_sweep:
            base = 12.0
    ...
    if displacement:
        points = 7.0 + (3.0 if displacement.has_fvg else 0.0)
        points = min(WEIGHTS["displacement"], points * min(1.15, displacement.body_atr / 1.15))
    ...
    if ote:
        ...
        if 0.62 <= retracement <= 0.79:
            _add(score, "ote", 8.0, f"價格位於 OTE 0.62-0.79 回撤區 ({retracement:.2f})")
        elif 0.50 <= retracement <= 0.86:
            _add(score, "ote", 4.0, f"價格接近 OTE 區但未進核心帶 ({retracement:.2f})")
        else:
            _add(score, "ote", 0.0, "")
    else:
        _add(score, "ote", 0.0, "")

    trendline = trendline_breakout(candles_4h, direction) if candles_4h else {"hit": False, "touches": 0}
    ...
    amd = amd_signal(candles_15m or candles_5m, direction) if (candles_15m or candles_5m) else {"score": 0.0, "phase": "missing"}
    amd_points = float(amd.get("score") or 0.0) * WEIGHTS["amd"]
    _add(score, "amd", amd_points, f"AMD phase={amd.get('phase')}" if amd_points else "")

    nexus = nexus_signal(candles_5m, direction) if candles_5m else {"score": 0.0, "reason": "missing"}
    ...
    entry_zone, stop, target, rr = _risk_setup(direction, candles_15m, candles_1h, price, fvg_zone, ote)
    score.entry_zone = entry_zone
    score.stop = stop
    score.target = target
    score.rr = rr
    if rr >= 2.0:
        _add(score, "risk_reward", 8.0, f"以最近流動性目標估算 RR={rr:.2f}")
    elif rr >= 1.5:
        _add(score, "risk_reward", 5.0, f"RR={rr:.2f}，可觀察但不是最漂亮")
    elif rr >= 1.1:
        _add(score, "risk_reward", 2.0, f"RR={rr:.2f} 偏低")
    else:
    ...
    _add(score, "market_quality", quality_points, "、".join(quality_notes) if quality_notes else "")
    return score
