...
def _evaluate_direction(
    ...
    if candles_15m or candles_1h:
        ...
        if rr >= 2.0:
            _add(score, "risk_reward", max(0.0, WEIGHTS["risk_reward"] - chase_penalty), f"以最近流動性目標估算 RR={rr:.2f}")
        elif rr >= 1.5:
            _add(score, "risk_reward", max(0.0, WEIGHTS["risk_reward"] * 0.68 - chase_penalty), f"RR={rr:.2f}，可觀察但不是最漂亮")
        elif rr >= 1.1:
            _add(score, "risk_reward", max(0.0, WEIGHTS["risk_reward"] * 0.28 - chase_penalty), f"RR={rr:.2f} 偏低")
        else:
    ...
    quality_max = max(3.0, quality_max)
