...
def _evaluate_direction(
    ...
    if candles_4h and len(candles_4h) >= 60:
        ...
        if trendline.get("hit"):
            ...
            _skip(score, "trendline", "未形成有效 2-3 觸點趨勢線破位，屬於未觸發加分項")
    else:
    ...
    if amd_candles and len(amd_candles) >= 90:
        ...
        if amd_points > 0:
            ...
            _skip(score, "amd", f"AMD 未進入可交易階段：{amd.get('phase')}")
    else:
    ...
    if candles_5m and len(candles_5m) >= 360:
        ...
        if nexus_points > 0:
            ...
            _skip(score, "nexus", f"Nexus/Silver Bullet 未觸發：{nexus.get('reason')}")
    else:
