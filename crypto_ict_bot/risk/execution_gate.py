...
def action_blockers(report: SymbolReport, limit: int | None = 3) -> list[str]:
    ...
    elif distance > gate["max_entry_distance_pct"]:
        blockers.append(f"距 entry {distance:.2f}% > {gate['max_entry_distance_pct']:.1f}%，等待回撤")
...
def evaluate_execution_gate(report: SymbolReport) -> dict[str, Any]:
    ...
    checks = {
        ...
        "rr": rr >= gate["min_rr"],
        "entry_distance": distance is not None and distance <= gate["max_entry_distance_pct"],
    ...
    if hard_gate_ok:
        code = "market" if distance is not None and distance <= 0.05 else "limit"
        return {
            "code": code,
            "label": "可以做",
            "reason": (
                f"execution_score={exec_score:.1f}，HTF/LTF/entry/RR/filter 全部過門檻，"
                f"距 entry {distance:.2f}%"
            ),
    ...
    elif distance is not None and distance <= gate["max_entry_distance_pct"] and score >= 62:
