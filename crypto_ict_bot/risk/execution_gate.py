...
def dynamic_entry_band_pct(report: SymbolReport, side: DirectionScore) -> float:
    ...
    return entry_distance_bands(report.symbol, atr_pct, spread_pct)["execution"]
...
def market_risk(report: SymbolReport) -> tuple[bool, str]:
    ...
    if btc_trend_pct_new is not None and direction in {"long", "short"}:
        ...
        if (
            not trend_aligned
            and abs(btc_trend_pct_new) >= conflict_threshold
            and (not healthy_heat or abs(btc_trend_pct_new) >= force_block_threshold)
        ):
...
def market_behavior_confirmation(report: SymbolReport, direction: str | None = None) -> dict[str, Any]:
    ...
    if directional_evidence_count <= 0:
        blockers.append("directional orderflow/context evidence required")
