...
def _trade_plan_audit(report: SymbolReport, side: DirectionScore, rr: float, required_rr: float) -> dict[str, Any]:
    ...
    missing: list[str] = []
    if report.selected_direction not in {"long", "short"}:
        missing.append("side")
    ...
    return {
        "complete": not missing and rr_ok,
        "missing": missing,
        "rr_ok": rr_ok,
        "origin": origin,
        "validity": validity,
...
def evaluate_execution_gate(report: SymbolReport) -> dict[str, Any]:
    ...
    severe_behavior_block = any(
        ...
        for blocker in behavior_blockers
        for needle in ("rejecting", "external bias", "derivatives hard block", "crowding", "exhaustion")
    )
    spread = _spread_pct(report)
    ...
    limit_behavior_ok = (not is_alt_family) or behavior_ok or strong_limit_override or liquid_flow_override
    limit_quality = {
    ...
    if liquid_flow_override or strong_limit_override:
    ...
    if market_ready:
        ...
        should_execute = True
        reason = (
            f"limit gate passed: complete {plan['origin']} plan, selection={score:.1f}, setup={setup_score:.1f}, "
            f"execution={exec_score:.1f}, RR {rr:.2f}R, distance {distance:.3f}% <= {dynamic_band * 2.5:.3f}%"
        )
    elif missed:
        code = "watch"
        status = "MISSED"
    ...
    checks = {
        ...
        "selected_direction": report.selected_direction in {"long", "short"},
        "score_gap": score_gap >= float(gate["min_score_gap"]),
        "complete_trade_plan": plan["complete"],
