...
def _derivatives_heat_profile(report: SymbolReport, direction: str) -> dict[str, Any]:
    ...
    if volume_ratio is not None:
        if 1.1 <= volume_ratio <= 4.5:
            ...
            if flow_alignment is not None and flow_alignment >= 0.58:
                ...
                exhaustion += min(16.0, (volume_ratio - 4.5) * 4.0)
    ...
    hard_block = False
    if funding_same_side and funding_abs >= 0.0008 and trend_confirmation < 35.0:
        hard_block = True
    if oi_change is not None and abs(oi_change) >= 35.0 and crowding >= 50.0 and trend_confirmation < 40.0:
        hard_block = True
    if crowding >= 75.0 and exhaustion >= 55.0:
        hard_block = True
    if (
    ...
    elif crowding_score >= 70.0 or exhaustion_score >= 70.0:
        state = "exhausted"
...
def derivative_risk(report: SymbolReport, direction: str) -> tuple[bool, str]:
    ...
    if not blocked and profile["crowding_score"] >= 82.0 and profile["trend_confirmation_score"] < 45.0:
        blocked = True
        warnings.append("crowding is extreme without enough trend confirmation")
    if not blocked and profile["exhaustion_score"] >= 82.0 and profile["crowding_score"] >= 55.0:
        blocked = True
...
def market_risk(report: SymbolReport) -> tuple[bool, str]:
    ...
    if atr_pct_new is not None:
        if atr_pct_new >= 6.5:
            new_warnings.append(f"ATR%={atr_pct_new:.2f} is extreme; wait for a fresh base/retest")
        elif atr_pct_new > 4.0 and (far_from_entry or btc_against_new or not healthy_heat):
            new_warnings.append(f"ATR%={atr_pct_new:.2f} is hot without enough confirmation; do not chase")
    if volume_ratio_new is not None:
        if volume_ratio_new >= 8.0 and not healthy_heat:
            new_warnings.append(f"volume spike={volume_ratio_new:.2f} is blow-off risk")
        elif volume_ratio_new > 5.5 and (far_from_entry or profile["exhaustion_score"] >= 45.0):
            new_warnings.append(f"volume spike={volume_ratio_new:.2f} needs retest confirmation")
    if btc_fast_pct_new is not None:
        ...
        if direction == "long":
            ...
            fast_aligned = btc_fast_pct_new <= 0
        if btc_against_new and abs(btc_fast_pct_new) >= 2.2:
            new_warnings.append(f"BTC fast move {btc_fast_pct_new:.2f}% is against selected side")
        elif abs(btc_fast_pct_new) >= 4.5 and not fast_aligned:
            new_warnings.append(f"BTC fast move {btc_fast_pct_new:.2f}% rejects selected side")
        elif abs(btc_fast_pct_new) >= 5.5 and not healthy_heat:
            new_warnings.append(f"BTC fast move {btc_fast_pct_new:.2f}% is unstable without confirming flow")
    if btc_trend_pct_new is not None and direction in {"long", "short"}:
        ...
        if not trend_aligned and abs(btc_trend_pct_new) >= 1.6:
            new_warnings.append(f"BTC context {btc_trend_pct_new:.2f}% conflicts with selected side")
    return bool(new_warnings), "; ".join(new_warnings)
    warnings: list[str] = []
    ...
    if btc_fast_pct is not None and (abs(btc_fast_pct) >= 3.5 or (btc_against and abs(btc_fast_pct) >= 2.2)):
...
def required_min_rr(report: SymbolReport, side: DirectionScore, diag: dict[str, Any] | None = None) -> float:
    ...
    if (
        ...
        return EXECUTION_GATE["strict_min_rr"]
    if (
        atr_pct >= 1.0
        and setup_score >= 74.0
        and exec_score >= 63.0
        and diag["ltf_trigger"] >= EXECUTION_GATE["limit_min_ltf_trigger"]
        and diag["entry_quality"] >= EXECUTION_GATE["limit_min_entry_quality"]
        ...
        return EXECUTION_GATE["scalp_min_rr"]
    return EXECUTION_GATE["min_rr"]
...
def action_blockers(report: SymbolReport, limit: int | None = 3) -> list[str]:
    ...
    if score < gate["limit_min_selection_score"]:
        blockers.append(f"selection_score {score:.1f} 未達限價門檻 {gate['limit_min_selection_score']:.0f}")
    if setup_score < gate["limit_min_setup_score"]:
        blockers.append(f"setup_score {setup_score:.1f} 未達限價門檻 {gate['limit_min_setup_score']:.0f}")
    if exec_score < gate["limit_min_execution_score"]:
        blockers.append(f"execution_score {exec_score:.1f} 未達限價門檻 {gate['limit_min_execution_score']:.0f}")
...
def evaluate_execution_gate(report: SymbolReport) -> dict[str, Any]:
    ...
    market_checks = {
        **common_checks,
        "market_rr": rr >= max(required_rr, EXECUTION_GATE["min_rr"]),
