...
def derivative_risk(report: SymbolReport, direction: str) -> tuple[bool, str]:
    ...
    if funding is not None:
        if direction == "long" and funding > 0.00035:
            warnings.append(f"funding={funding:.5f}; crowded long funding")
            if funding >= 0.0008 or ("long_crowded" in flags and (oi_change or 0.0) >= 10.0):
                blocked = True
        if direction == "short" and funding < -0.00035:
            warnings.append(f"funding={funding:.5f}; crowded short funding")
            if funding <= -0.0008 or ("short_crowded" in flags and (oi_change or 0.0) >= 10.0):
                blocked = True
    if oi_change is not None and abs(oi_change) >= 18:
        warnings.append(f"OI 1h change {oi_change:.2f}% is hot")
        if abs(oi_change) >= 30 or "derivatives_hot" in flags:
            blocked = True
    if direction == "long" and "stop_hunt_risk_long" in flags:
        warnings.append("external liquidity below is close; long stop-hunt risk elevated")
    if direction == "short" and "stop_hunt_risk_short" in flags:
        warnings.append("external liquidity above is close; short stop-hunt risk elevated")
    if direction in {"long", "short"} and external_bias in {"long", "short"} and external_bias != direction:
        warnings.append(f"external orderflow bias={external_bias} conflicts with selected {direction}")
        if external_confidence >= 74.0 and _context_direction_score(external_context, direction) < 48.0:
            blocked = True
    return blocked, "; ".join(warnings)
...
def derivatives_context(report: SymbolReport) -> dict[str, Any]:
    ...
    public_score = _public_derivatives_score(public)
    ...
    reason_parts = []
    if has_public:
        reason_parts.append(f"public funding/OI score {public_score:.1f}{time_note}")
    if has_context:
        reason_parts.append(
...
def _public_derivatives_score(public: dict[str, Any]) -> float | None:
    funding = _as_float(public.get("funding_rate"))
    open_interest = _as_float(public.get("open_interest"))
    oi_change = _as_float(public.get("open_interest_change_pct"))
...
def required_min_rr(report: SymbolReport, side: DirectionScore, diag: dict[str, Any] | None = None) -> float:
    ...
    if diag["market_overheated"] or report.quote_volume_24h < 20_000_000:
