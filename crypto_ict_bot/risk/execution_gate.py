...
def _mover_structure_behavior_proxy(
    report: SymbolReport,
    side: DirectionScore,
    ...
    if profile not in {"active_mover", "hot_mover", "extreme_mover"}:
        return {"ok": False, "reason": "not a high-vol mover", "checks": {}}

    direction_ok = report.selected_direction in {"long", "short"}
    quote_volume = _as_float(getattr(report, "quote_volume_24h", None)) or 0.0
    distance = entry_distance_pct(report.price, side.entry_zone)
    band = dynamic_entry_band_pct(report, side)
    anchor_score = _as_float(metrics.get("entry_anchor_score")) or 0.0
    anchor_ok = bool(metrics.get("entry_anchor_ok")) and anchor_score >= (58.0 if kind == "altcoin" else 56.0)
    mover_permission = bool(metrics.get("mover_execution_permission"))
    mover_chase = bool(metrics.get("mover_chase_risk"))
    flow_alignment = _as_float(heat.get("flow_alignment"))
    depth_alignment = _as_float(heat.get("depth_alignment"))
    ...
    htf_context = _as_float(buckets.get("htf_context")) or 0.0
    ltf_trigger = _as_float(buckets.get("ltf_confirmation")) or 0.0
    entry_quality = _as_float(buckets.get("entry_location")) or 0.0
    risk_quality = _as_float(buckets.get("risk_plan")) or 0.0
    score = side_score(side)
    setup_score = _as_float(getattr(side, "setup_score", None)) or score
    exec_score = execution_score(side, score)
    rr = _as_float(getattr(side, "rr", None)) or 0.0
    three_day_range = _as_float(metrics.get("three_day_range_pct")) or 0.0
    avg_range = _as_float(metrics.get("three_day_avg_range_pct")) or 0.0

    distance_mult = 2.15 if profile == "extreme_mover" else 2.35 if profile == "hot_mover" else 2.55
    liquidity_floor = 18_000_000.0 if kind == "altcoin" else 35_000_000.0
    score_floor = 52.0 if profile == "extreme_mover" else 54.0
    setup_floor = 48.0 if profile == "extreme_mover" else 50.0
    exec_floor = 36.0 if profile == "extreme_mover" else 40.0
    ltf_floor = 38.0 if profile == "extreme_mover" else 42.0
    entry_floor = 42.0 if profile == "extreme_mover" else 45.0
    risk_floor = 55.0 if profile == "extreme_mover" else 58.0
    hostile_flow = flow_alignment is not None and flow_alignment <= (0.16 if kind == "altcoin" else 0.18)
    hostile_depth = depth_alignment is not None and depth_alignment <= (0.24 if kind == "altcoin" else 0.26)
    severe_hostile_microstructure = hostile_flow and hostile_depth
    heat_not_exhausted = crowding < (74.0 if profile == "extreme_mover" else 70.0) and exhaustion < (
        74.0 if profile == "extreme_mover" else 70.0
    )
    structure_ok = (
        htf_context >= 48.0
        or entry_quality >= entry_floor + 10.0
        or anchor_score >= 82.0
    )
    has_vol_context = three_day_range > 0.0 and avg_range > 0.0
    checks = {
        "direction": direction_ok,
        "mover_permission": mover_permission,
        "no_chase": not mover_chase,
        "anchor": anchor_ok,
        "distance": distance is not None and distance <= band * distance_mult,
        "liquidity": quote_volume >= liquidity_floor,
        "score": score >= score_floor,
        "setup_score": setup_score >= setup_floor,
        "execution_score": exec_score >= exec_floor,
        "structure": structure_ok,
        "ltf_trigger": ltf_trigger >= ltf_floor or anchor_score >= 82.0,
        "entry_quality": entry_quality >= entry_floor or anchor_score >= 82.0,
        "risk_quality": risk_quality >= risk_floor or rr >= 1.55,
        "rr": rr >= 1.35,
        "vol_context": has_vol_context,
        "not_hard_heat": not bool(heat.get("hard_block")),
        "heat_not_exhausted": heat_not_exhausted,
        "not_severe_hostile_microstructure": not severe_hostile_microstructure,
    }
    ok = all(checks.values())
    reason = (
        f"{profile} structure proxy: anchor={anchor_score:.0f}, "
        f"distance={distance if distance is not None else -1:.2f}%/{band:.2f}%, "
        f"3d_range={three_day_range:.1f}%, flow={flow_alignment if flow_alignment is not None else 'na'}, "
        f"depth={depth_alignment if depth_alignment is not None else 'na'}"
    )
...
def market_behavior_confirmation(report: SymbolReport, direction: str | None = None) -> dict[str, Any]:
    ...
    if liquid_hot_alt:
        support_floor = 1.80
    mover_proxy = _mover_structure_behavior_proxy(report, side, metrics, heat, kind)
    if mover_proxy["ok"]:
        evidence_count = max(evidence_count, required_evidence)
        directional_evidence_count = max(directional_evidence_count, required_directional_evidence)
        directional_support = max(directional_support, support_floor + 0.10)
        reasons.append(str(mover_proxy["reason"]))
        soft_orderflow_prefixes = (
            "taker flow weak/rejecting",
            "orderbook depth against",
        )
        blockers = [
            blocker
            for blocker in blockers
            if not any(blocker.startswith(prefix) for prefix in soft_orderflow_prefixes)
        ]
    ...
    return {
        ...
        "blockers": blockers,
        "mover_structure_proxy": bool(mover_proxy["ok"]),
        "mover_structure_proxy_reason": mover_proxy["reason"],
        "mover_structure_proxy_checks": mover_proxy["checks"],
    }
...
def evaluate_execution_gate(report: SymbolReport) -> dict[str, Any]:
    ...
    quant_composite = _as_float(side.market_metrics.get("quant_composite_score")) if isinstance(side.market_metrics, dict) else None
    quant_entry_precision = _as_float(side.market_metrics.get("quant_entry_precision_score")) if isinstance(side.market_metrics, dict) else None
    quant_derivatives = _as_float(side.market_metrics.get("quant_derivatives_score")) if isinstance(side.market_metrics, dict) else None
    quant_no_chase = _as_float(side.market_metrics.get("quant_no_chase_score")) if isinstance(side.market_metrics, dict) else None
    quant_crowding = _as_float(side.market_metrics.get("quant_crowding_risk")) if isinstance(side.market_metrics, dict) else None
    strategy_hint = str(side.market_metrics.get("strategy_hint") or "") if isinstance(side.market_metrics, dict) else ""
    scorecard_hot_plan_ok = (
        is_alt_family
        and strategy_hint in {"hot_mover_structural_retest", "mover_squeeze_reversal", "derivatives_squeeze_reversal", "alt_beta_pullback"}
        and (quant_composite or 0.0) >= 61.0
        and (quant_entry_precision or 0.0) >= 55.0
        and (quant_derivatives or 0.0) >= 55.0
        and (quant_no_chase or 0.0) >= 60.0
        and (quant_crowding or 0.0) < 72.0
        and not mover_chase_risk
        and (entry_anchor_ok or (quant_entry_precision or 0.0) >= 66.0)
    )
    ...
    if diag["derivative_blocked"] and not scorecard_hot_plan_ok:
        hard_blockers.append(f"derivatives hard risk: {diag['derivative_warning']}")
    elif diag["derivative_blocked"]:
        soft_warnings.append(f"derivatives risk contained by scorecard route: {diag['derivative_warning']}")
    if diag["market_overheated"]:
        hard_blockers.append(f"market hard risk: {diag['market_warning']}")
    if hard_behavior_block and not scorecard_hot_plan_ok:
        hard_blockers.append("severe orderflow/derivatives rejection: " + "; ".join(behavior_blockers[:2]))
    elif hard_behavior_block:
        soft_warnings.append("severe orderflow noise downgraded by scorecard structural route: " + "; ".join(behavior_blockers[:2]))
    if mover_chase_risk:
    ...
    if instrument == "altcoin":
        ...
        if scorecard_hot_plan_ok:
            limit_distance_budget = max(limit_distance_budget, dynamic_band * 3.15, entry_zone_width_pct * 1.65, 0.72)
        ...
        if scorecard_hot_plan_ok:
            limit_distance_budget = max(limit_distance_budget, dynamic_band * 2.90, entry_zone_width_pct * 1.50, 0.58)
        limit_distance_budget = min(limit_distance_budget, distance_bands["caution"])
    ...
    if scorecard_hot_plan_ok:
        limit_quality["entry_quality"] = limit_quality["entry_quality"] or diag["entry_quality"] >= float(gate["limit_min_entry_quality"]) - 8.0
        limit_quality["market_behavior_for_limit"] = True
        limit_quality["risk_quality"] = limit_quality["risk_quality"] or plan["rr_ok"]
    limit_ready = (
        not hard_blockers
        and plan["complete"]
    ...
    mover_structural_watch = (
        is_alt_family
        and report.selected_direction in {"long", "short"}
        and mover_profile in {"active_mover", "hot_mover", "extreme_mover"}
        and bool(side.market_metrics.get("mover_execution_permission"))
        and not mover_chase_risk
        and entry_anchor_ok
        and plan["structurally_complete"]
        and plan["rr_ok"]
        and distance is not None
        and distance <= distance_bands["stale"]
        and quote_volume >= (18_000_000.0 if instrument == "altcoin" else 35_000_000.0)
        and score >= 52.0
        and setup_score >= 48.0
        and exec_score >= 35.0
    )
    market_checks = {
    ...
    elif mover_structural_watch and not hard_blockers:
        code = "watch"
        status = "WATCH"
        should_execute = False
        reason = (
            f"{mover_profile} structural watch: 3-day volatility model accepts the setup as watch-only; "
            f"needs stronger LTF/orderflow confirmation before execution"
        )
    elif hard_blockers:
        code = "avoid"
    ...
    checks = {
        ...
        "derivatives_context": diag["derivatives_context_score"] >= float(gate["min_derivatives_context"]),
        "derivatives_not_overheated": (not diag["derivative_blocked"]) or scorecard_hot_plan_ok,
        "market_not_overheated": not diag["market_overheated"],
        "market_behavior": behavior_ok,
        "limit_behavior_ok": limit_behavior_ok,
        "orderflow_not_hard_rejected": (not hard_behavior_block) or scorecard_hot_plan_ok,
        "scorecard_hot_plan_ready": scorecard_hot_plan_ok,
        "quant_composite_score": round(quant_composite, 2) if quant_composite is not None else None,
        "quant_entry_precision_score": round(quant_entry_precision, 2) if quant_entry_precision is not None else None,
        "quant_derivatives_score": round(quant_derivatives, 2) if quant_derivatives is not None else None,
        "quant_no_chase_score": round(quant_no_chase, 2) if quant_no_chase is not None else None,
        ...
        "mover_structural_watch": "yes" if mover_structural_watch else "no",
