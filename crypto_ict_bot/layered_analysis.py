...
def _prediction_layer(
    ...
    long_score = _weighted_engine_score(long_engines)
    short_score = _weighted_engine_score(short_engines)
...
def _trade_plan_layer(
    ...
    entry_price = _midpoint(side.entry_zone)
    ...
    if prediction["prediction_direction"] == "neutral" or setup["setup_type"] == "no_clear_setup":
        status = "incomplete"
    elif not complete:
        status = "incomplete"
...
def _engine_bundle(report: SymbolReport, side: DirectionScore, direction: str, regime: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "derivatives_engine": _derivatives_engine(heat, direction),
        "orderflow_engine": _orderflow_engine(behavior, direction),
...
def _weighted_engine_score(engines: dict[str, dict[str, Any]]) -> float:
    total = 0.0
    for name, weight in PREDICTION_ENGINE_WEIGHTS.items():
