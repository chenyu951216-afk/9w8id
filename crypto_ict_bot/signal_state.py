...
from .instrument_classifier import entry_distance_bands
...
def apply_signal_stability(reports: list[SymbolReport], path: Path = STATE_PATH) -> dict[str, Any]:
    ...
    for symbol, previous in list(symbols_state.items()):
        ...
        current["signal_present"] = False
        current["signal_seen_count"] = 0
        current["signal_absent_count"] = int(current.get("signal_absent_count", 0) or 0) + 1
        if current.get("execution_state") in {"ARMED", "EXECUTABLE"}:
            current["execution_state"] = "WINDOW_CLOSED"
            current["can_execute_now"] = False
            current["executable_miss_count"] = int(current.get("executable_miss_count", 0) or 0) + 1
        if current["miss_count"] >= 6 and current.get("status") not in {"invalid", "expired", "missed"}:
            ...
            current["execution_state"] = "CLOSED"
            current["can_execute_now"] = False
        symbols_state[symbol] = current

    state["updated_at"] = now.isoformat()
    state["statistics"] = build_statistics(symbols_state)
    _attach_setup_statistics(reports, state["statistics"])
    save_signal_state(state, path)
    return state


def _attach_setup_statistics(reports: list[SymbolReport], statistics: dict[str, Any]) -> None:
    tag_stats = statistics.get("setup_tag_stats", {}) if isinstance(statistics, dict) else {}
    combo_stats = statistics.get("setup_combo_stats", {}) if isinstance(statistics, dict) else {}
    for report in reports:
        signal_state = report.metadata.get("signal_state", {})
        tags = list(signal_state.get("setup_tags", []))
        combo = _combo_key(tags)
        setup_stats = {
            "combo": combo,
            "combo_stats": combo_stats.get(combo, {}),
            "tag_stats": {tag: tag_stats.get(tag, {}) for tag in tags if tag in tag_stats},
        }
        report.metadata["setup_stats"] = setup_stats
        if isinstance(signal_state, dict):
            signal_state["setup_stats"] = setup_stats
...
def _update_report_state(report: SymbolReport, previous: dict[str, Any], now: datetime) -> dict[str, Any]:
    ...
    signal_present = direction in {"long", "short"} and current_score >= 60
    signal_seen_count = int(previous.get("signal_seen_count", 0) or 0) if same_direction and signal_present else 0
    signal_absent_count = int(previous.get("signal_absent_count", 0) or 0) if same_direction and not signal_present else 0
    if signal_present:
        signal_seen_count += 1
        signal_absent_count = 0
    else:
        signal_seen_count = 0
        signal_absent_count += 1

    raw_executable = bool(raw.get("should_execute")) and raw.get("code") in {"market", "limit"}
    executable_confirm_count = (
        int(previous.get("executable_confirm_count", 0) or 0) if same_direction and raw_executable else 0
    )
    executable_miss_count = int(previous.get("executable_miss_count", 0) or 0) if same_direction else 0
    if raw_executable:
        executable_confirm_count += 1
        executable_miss_count = 0
    elif same_direction and previous.get("execution_state") in {"ARMED", "EXECUTABLE"}:
        executable_miss_count += 1
    else:
        executable_miss_count = 0
    ...
    distance_bands = _distance_bands(report, side)
    ...
    if distance is not None and distance > distance_bands["missed"] and status not in {"invalid", "expired"}:
        status = "missed"
    ...
    if _is_mover_structural_watch(raw) and status in {"invalid", "expired"}:
        status = "watching" if current_score >= 70 else "warning"
        invalid_reason = [reason for reason in invalid_reason if "240" not in str(reason)]

    market_warning = _has_market_warning(diag, warning_reason)
    liquidity_warning = _has_liquidity_warning(report, warning_reason)
    priority_level = _priority_level(current_score, side.execution_score, confirm_count, score_trend, status, market_warning, liquidity_warning, distance, diag)
    htf_bias = _htf_bias(report, diag)
    setup_tags = _dedupe(_setup_tags(side) + _risk_tags(report, diag, distance))
    future_validation = _update_future_validation(report, previous, now, direction, current_score, side, diag, setup_tags, distance)
    ...
    can_execute_now = execution_state == "EXECUTABLE" and stable_code in {"market", "limit"}
    ...
    output = {
        ...
        "signal_seen_count": signal_seen_count,
        "signal_absent_count": signal_absent_count,
        "executable_confirm_count": executable_confirm_count,
        "executable_miss_count": executable_miss_count,
    ...
    report.metadata["signal_state"] = {
        **output,
        "stable_action": {
            "code": stable_code,
            "label": _stable_action_label(stable_code, raw),
...
def _distance_bands(report: SymbolReport, side: Any) -> dict[str, float]:
    metrics = getattr(side, "market_metrics", {}) or {}
    bands = entry_distance_bands(report.symbol, _as_float(metrics.get("atr_pct")))
    adaptive = _as_float(metrics.get("adaptive_entry_band_pct"))
    if adaptive is not None:
        bands = dict(bands)
        bands["execution"] = max(float(bands["execution"]), adaptive)
        bands["caution"] = max(float(bands["caution"]), adaptive * 2.4)
        bands["stale"] = max(float(bands["stale"]), adaptive * 4.8)
    return bands
...
def _warning_reasons(report: SymbolReport, side: Any, diag: dict[str, Any], score_trend: str, distance: float | None) -> list[str]:
    ...
    bands = _distance_bands(report, side)
    ...
    if distance is not None and distance > bands["caution"]:
...
def _invalid_reasons(
    ...
    if distance is not None and distance > bands["missed"]:
...
def _priority_level(
    ...
    exec_score = execution_score if execution_score is not None else score
    core_ready = (
        diag.get("htf_context", 0) >= 60
        and diag.get("ltf_trigger", 0) >= 65
        and diag.get("entry_quality", 0) >= 65
        and diag.get("risk_reward_quality", 0) >= 60
    )
    if score >= 85 and exec_score >= 82 and confirm_count >= 2 and core_ready and score_trend not in {"weakening", "sharp_drop"} and not market_warning and distance is not None and distance <= 0.3:
        level = "A"
    elif score >= 75 and exec_score >= 68 and confirm_count >= 1 and core_ready:
        level = "B"
    elif score >= 65:
        level = "C"
    else:
        level = "D"
    if level in {"A", "B", "C"} and (score_trend in {"weakening", "sharp_drop"} or market_warning or liquidity_warning or not distance_ok):
...
def _stable_action_code(priority_level: str, status: str, raw: dict[str, Any]) -> str:
    if raw.get("should_execute") is True and raw.get("code") in {"market", "limit"}:
        return raw["code"]
    if _is_mover_structural_watch(raw) and raw.get("execution_status") != "MISSED":
        return "watch"
    if priority_level == "X" or status in {"invalid", "expired", "missed"}:
        return "avoid"
    if raw.get("code") == "avoid":
        return "avoid"
...
def _stable_action_label(stable_code: str, raw: dict[str, Any]) -> str:
    raw_label = raw.get("label")
    if stable_code == "watch" and raw_label in {"錯過 / 不追", "待確認"}:
        return raw_label
    if stable_code == "avoid" and raw_label in {"禁止", "不能做"}:
        return raw_label
    return ACTION_LABELS[stable_code]


def _is_mover_structural_watch(raw: dict[str, Any]) -> bool:
    gate = raw.get("gate_checks") if isinstance(raw.get("gate_checks"), dict) else {}
    return raw.get("code") == "watch" and gate.get("mover_structural_watch") == "yes"


def _execution_state(
    raw_executable: bool,
    signal_present: bool,
    signal_seen_count: int,
    signal_absent_count: int,
    executable_miss_count: int,
    status: str,
    stable_code: str,
) -> str:
    if status in {"invalid", "expired", "missed"}:
        if not (stable_code == "watch" and signal_present and status != "missed"):
            return "CLOSED"
    if stable_code == "avoid":
        return "BLOCKED" if signal_present else "CLOSED"
    if raw_executable and stable_code in {"market", "limit"}:
        return "EXECUTABLE"
    if executable_miss_count > 0:
        if executable_miss_count <= 2 and signal_present and stable_code == "watch":
            return "MANAGE"
        return "WINDOW_CLOSED"
    if signal_present and signal_seen_count >= 1:
        return "ARMED"
    if signal_absent_count > 0:
        return "COOLDOWN"
    return "SCOUT"
...
def _stable_reason(
    ...
    if _is_mover_structural_watch(raw):
        return f"高波動山寨結構觀察：{raw.get('reason') or '等待低週期與訂單流確認'}"
    if raw.get("code") == "avoid":
        gate = raw.get("gate_checks") if isinstance(raw.get("gate_checks"), dict) else {}
        mover_profile = str(gate.get("mover_profile") or "")
        if mover_profile in {"active_mover", "hot_mover", "extreme_mover"}:
            return f"高波動山寨模型：{raw.get('label')} - {raw.get('reason')}"
        return f"風控未通過：{raw.get('label')} - {raw.get('reason')}"
    if raw.get("code") == "watch" and raw.get("label") in {"錯過 / 不追", "待確認"}:
        return str(raw.get("reason") or "Execution gate waiting for confirmation.")
    if raw.get("should_execute") is True and raw.get("code") in {"market", "limit"}:
        return f"Execution gate passed: {raw.get('label')}；{raw.get('reason')}"
    if priority_level == "A":
...
def _update_future_validation(
    ...
    if isinstance(previous_validation, dict) and previous_validation.get("signal_price"):
        ...
        validation = {
            ...
            "entry_distance_pct": distance,
            "funding": _derivative_snapshot(report).get("funding"),
            "oi_change": _derivative_snapshot(report).get("oi_change"),
            "btc_trend": side.market_metrics.get("btc_trend_pct"),
            "atr_pct": side.market_metrics.get("atr_pct"),
            "volume_ratio": side.market_metrics.get("volume_ratio"),
    ...
    validation.setdefault("score", validation.get("score_at_signal", round(current_score, 2)))
    validation.setdefault("setup_tags", setup_tags)
    validation.setdefault("entry_zone", side.entry_zone)
    validation.setdefault("stop", side.stop)
    validation.setdefault("take_profits", side.take_profits)
    validation.setdefault("entry_distance_pct", distance)
    derivative = _derivative_snapshot(report)
    validation.setdefault("funding", derivative.get("funding"))
    validation.setdefault("oi_change", derivative.get("oi_change"))
    validation.setdefault("btc_trend", side.market_metrics.get("btc_trend_pct"))
    validation.setdefault("atr_pct", side.market_metrics.get("atr_pct"))
    validation.setdefault("volume_ratio", side.market_metrics.get("volume_ratio"))
    validation.setdefault("entry_observed", False)
    validation.setdefault("entry_observed_time", None)
    ...
    validation["mfe"] = validation["max_favorable_move"]
    validation["mae"] = validation["max_adverse_move"]
    _update_touch_order(validation, direction, report.price, now)
...
def _risk_tags(report: SymbolReport, diag: dict[str, Any], distance: float | None) -> list[str]:
    tags: list[str] = []
    bands = _distance_bands(report, selected_side(report))
    if distance is not None and distance > bands["caution"]:
        tags.append("entry_distance > dynamic caution band")
    warning = str(diag.get("derivative_warning") or "")
    if "funding" in warning.lower():
        tags.append("funding 過熱")
    if "OI" in warning or "oi" in warning.lower():
        tags.append("OI 1h 變動過大")
    if diag.get("market_api_quality", 100) < 45:
        tags.append("BTC 反向")
    side = selected_side(report)
    if side.market_metrics.get("btc_against"):
        tags.append("BTC 反向")
    return _dedupe(tags)


def _derivative_snapshot(report: SymbolReport) -> dict[str, float | None]:
    ...
    return {
        "funding": _as_float(public.get("funding_rate")) if isinstance(public, dict) else None,
        "oi_change": _as_float(public.get("open_interest_change_pct")) if isinstance(public, dict) else None,
    }


def _update_touch_order(validation: dict[str, Any], direction: str, price: float, now: datetime) -> None:
    if validation.get("first_touch"):
        return
    entry_zone = validation.get("entry_zone")
    if isinstance(entry_zone, (list, tuple)) and len(entry_zone) == 2:
        low = _as_float(entry_zone[0])
        high = _as_float(entry_zone[1])
        if low is not None and high is not None and low <= price <= high and not validation.get("entry_observed"):
            validation["entry_observed"] = True
            validation["entry_observed_time"] = now.isoformat()
    if not validation.get("entry_observed"):
        return
    stop = _as_float(validation.get("stop"))
    take_profits = validation.get("take_profits") or []
    tp1 = None
    if isinstance(take_profits, list) and take_profits:
        first = take_profits[0] if isinstance(take_profits[0], dict) else {}
        tp1 = _as_float(first.get("price"))
    touched_tp1 = False
    touched_sl = False
    if direction == "long":
        touched_tp1 = tp1 is not None and price >= tp1
        touched_sl = stop is not None and price <= stop
    elif direction == "short":
        touched_tp1 = tp1 is not None and price <= tp1
        touched_sl = stop is not None and price >= stop
    if touched_tp1:
        validation["first_touch"] = "TP1"
        validation["tp1_hit_first"] = True
        validation["sl_hit_first"] = False
        validation["first_touch_time"] = now.isoformat()
    elif touched_sl:
        validation["first_touch"] = "SL"
        validation["tp1_hit_first"] = False
        validation["sl_hit_first"] = True
        validation["first_touch_time"] = now.isoformat()
...
def _failed_reason(
    ...
    if distance is not None and distance > _distance_bands(report, side)["caution"]:
...
def build_statistics(symbols_state: dict[str, Any]) -> dict[str, Any]:
    ...
    stats = {
        ...
        "setup_combo_stats": _setup_combo_stats(records),
        "calibration_rules": _calibration_rules(records),
        "weakness_stats": {
...
def _setup_tag_stats(records: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for tag in item.get("setup_tags", []):
            bucket = output.setdefault(
                tag,
                {
                    ...
                    "tp1_first": 0,
                    "sl_first": 0,
                    "high_score_checked": 0,
                    "high_score_reversed": 0,
                },
            )
            bucket["total"] += 1
            if correct is True:
            ...
            if correct is False or item.get("priority_level") == "X":
                bucket["failed"] += 1
            mfe = _as_float(validation.get("max_favorable_move"))
            mae = _as_float(validation.get("max_adverse_move"))
            if mfe is not None:
                bucket["mfe_values"].append(mfe)
            if mae is not None:
                bucket["mae_values"].append(mae)
            if validation.get("tp1_hit_first") is True:
                bucket["tp1_first"] += 1
            if validation.get("sl_hit_first") is True:
                bucket["sl_first"] += 1
            score_at_signal = _as_float(validation.get("score") or validation.get("score_at_signal"))
            after_3 = validation.get("after_3_candles")
            after_6 = validation.get("after_6_candles")
            high_score_result = after_6 if isinstance(after_6, dict) else after_3
            if score_at_signal is not None and score_at_signal >= 82 and isinstance(high_score_result, dict):
                bucket["high_score_checked"] += 1
                if high_score_result.get("direction_correct") is False:
                    bucket["high_score_reversed"] += 1
    for bucket in output.values():
        ...
        bucket["average_mfe"] = _avg(bucket.pop("mfe_values"))
        bucket["average_mae"] = _avg(bucket.pop("mae_values"))
        touch_total = bucket["tp1_first"] + bucket["sl_first"]
        bucket["tp1_first_rate"] = round(bucket["tp1_first"] / touch_total * 100.0, 1) if touch_total else None
        bucket["sl_first_rate"] = round(bucket["sl_first"] / touch_total * 100.0, 1) if touch_total else None
        checked = bucket["high_score_checked"]
        bucket["high_score_reversal_rate"] = round(bucket["high_score_reversed"] / checked * 100.0, 1) if checked else None
    return output
...
def _combo_key(tags: list[str]) -> str:
    important = [
        ...
        "funding 過熱",
        "OI 1h 變動過大",
        "entry_distance > dynamic caution band",
        "BTC 反向",
    ]
    selected = [tag for tag in important if tag in tags]
    return " + ".join(selected) if selected else "Other"


def _setup_combo_stats(records: list[dict[str, Any]]) -> dict[str, Any]:
    combo_records: dict[str, list[dict[str, Any]]] = {}
    for item in records:
        tags = list(item.get("setup_tags", []))
        combo_records.setdefault(_combo_key(tags), []).append(item)
        if {"Liquidity Sweep", "MSS/BOS", "FVG", "OTE"}.issubset(set(tags)):
            combo_records.setdefault("Liquidity Sweep + MSS/BOS + FVG + OTE", []).append(item)
        if "Liquidity Sweep" in tags and "FVG" in tags and "OTE" not in tags:
            combo_records.setdefault("Sweep + FVG 但無 OTE", []).append(item)
        if "AMD" in tags and len([tag for tag in tags if tag in {"Liquidity Sweep", "MSS/BOS", "FVG", "OTE"}]) < 2:
            combo_records.setdefault("AMD only", []).append(item)
        if "Nexus" in tags and len([tag for tag in tags if tag in {"Liquidity Sweep", "MSS/BOS", "FVG", "OTE"}]) < 2:
            combo_records.setdefault("Nexus only", []).append(item)
    return {combo: _setup_tag_stats([*items]).get(combo, _combo_summary(items)) for combo, items in combo_records.items()}


def _combo_summary(items: list[dict[str, Any]]) -> dict[str, Any]:
    correct = 0
    failed = 0
    ...
    for item in items:
        validation = item.get("future_validation", {})
        if validation.get("direction_correct") is True:
            correct += 1
        if validation.get("direction_correct") is False or item.get("priority_level") == "X":
            failed += 1
        mfe_values.append(_as_float(validation.get("max_favorable_move")))
        mae_values.append(_as_float(validation.get("max_adverse_move")))
    denominator = correct + failed
    return {
        "total": len(items),
        "correct": correct,
        "failed": failed,
        "accuracy": round(correct / denominator * 100.0, 1) if denominator else None,
        "average_mfe": _avg(mfe_values),
        "average_mae": _avg(mae_values),
    }


def _calibration_rules(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rules: list[dict[str, Any]] = []
    for name, stats in _setup_tag_stats(records).items():
        total = int(stats.get("total") or 0)
        accuracy = stats.get("accuracy")
        avg_mfe = stats.get("average_mfe")
        avg_mae = stats.get("average_mae")
        sl_rate = stats.get("sl_first_rate")
        if total < 8:
            continue
        reasons: list[str] = []
        if accuracy is not None and accuracy < 45:
            reasons.append(f"win_rate {accuracy:.1f}% < 45%")
        if avg_mfe is not None and avg_mae is not None and avg_mae > avg_mfe:
            reasons.append("MAE > MFE")
        if sl_rate is not None and sl_rate >= 50:
            reasons.append(f"SL first {sl_rate:.1f}%")
        if reasons:
            rules.append({"setup": name, "action": "downweight", "reasons": reasons})
    return rules
