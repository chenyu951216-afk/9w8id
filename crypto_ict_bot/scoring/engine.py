...
def _validation_adjustment_from_stats(score: DirectionScore, stats: dict[str, Any]) -> tuple[float, float, float, list[str]]:
    ...
    for tag in tags:
        ...
        if total >= 8 and avg_mfe is not None and avg_mae is not None and float(avg_mae) > float(avg_mfe):
            ...
            reasons.append(f"{tag} MAE 大於 MFE，降低 execution_score")
        ...
        if total >= 8 and high_score_reversal_rate is not None and float(high_score_reversal_rate) >= 50.0:
            ...
            execution_cap = min(execution_cap, 64.0)
            reasons.append(f"{tag} 高分後 3-6 根 K 常反向，不列 A 級")
    if raw_penalty > 14.0:
        penalty = min(penalty, 14.0)
        reasons.append("相關 setup tag 同時出現，歷史校準扣分封頂 14 分，避免重複懲罰同一結構")
...
def _calibrate_score(score: DirectionScore, ticker: Ticker, price: float) -> None:
    ...
    atr_pct = score.market_metrics.get("atr_pct")
    volume_ratio = score.market_metrics.get("volume_ratio")
    if isinstance(atr_pct, (int, float)):
        if atr_pct < 0.12:
            ...
            adjustments.append(f"ATR%={atr_pct:.2f} 過熱，禁止追小幣")
        elif atr_pct > 2.4:
            execution_cap = min(execution_cap, 74.0)
            execution_penalty += 3.0
            adjustments.append(f"ATR%={atr_pct:.2f} 偏熱，降低 execution_score")
    if isinstance(volume_ratio, (int, float)) and volume_ratio > 5.5:
        execution_cap = min(execution_cap, 66.0)
        execution_penalty += 6.0
        adjustments.append(f"volume spike {volume_ratio:.2f} 倍過熱，避免追高/追空")
...
def _risk_setup(
    ...
    stop_mult = _intraday_stop_atr_mult(atr_pct)
    ...
    if direction == "long":
        ...
        stop = min(base_stop, zone_low) - local_atr * stop_mult
        risk = max(entry - stop, price * 0.001)
        liquidity_target = buy_side if buy_side and buy_side > entry else None
        target = liquidity_target if liquidity_target and liquidity_target >= entry + risk * min_liquidity_rr else entry + risk * default_target_rr
        ...
        stop = max(base_stop, zone_high) + local_atr * stop_mult
