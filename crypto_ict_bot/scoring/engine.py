...
def _intraday_stop_atr_mult(atr_pct: float, symbol: str | None = None) -> float:
...
def _intraday_min_stop_risk_atr_mult(atr_pct: float, symbol: str | None = None) -> float:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
        if atr_pct >= 4.0:
            return 1.05
...
def _intraday_max_stop_risk_atr_mult(atr_pct: float, symbol: str | None = None) -> float:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
        if atr_pct >= 4.0:
            return 2.40
...
def _intraday_max_stop_pct(atr_pct: float, symbol: str | None = None) -> float:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
        if atr_pct >= 4.0:
            return 5.2
...
def _intraday_default_target_rr(atr_pct: float, symbol: str | None = None) -> float:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
...
def _intraday_min_liquidity_rr(atr_pct: float, symbol: str | None = None) -> float:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
...
def _target_entry_zone_width(symbol: str | None, price: float, local_atr: float, atr_pct: float, current_width: float) -> float:
    kind = _instrument_kind(symbol)
...
def _expanded_entry_zone(
    ...
    target_width = _target_entry_zone_width(symbol, price, local_atr, atr_pct, current_width)
...
def _take_profit_rr_bounds(symbol: str | None) -> tuple[float, float]:
    kind = _instrument_kind(symbol)
    if kind == "altcoin":
        return 2.75, 4.20
    if kind == "large_altcoin":
...
def _bounded_structural_stop(
    ...
    stop_buffer = local_atr * _intraday_stop_atr_mult(atr_pct, symbol)
    min_risk = max(abs(entry) * 0.0015, local_atr * _intraday_min_stop_risk_atr_mult(atr_pct, symbol))
    max_risk = min(
        local_atr * _intraday_max_stop_risk_atr_mult(atr_pct, symbol),
        abs(entry) * _intraday_max_stop_pct(atr_pct, symbol) / 100.0,
    )
...
def _risk_setup(
    ...
    if fvg_zone is not None or ote is not None:
        zone_low, zone_high = _expanded_entry_zone(direction, zone_low, zone_high, price, local_atr, atr_pct, symbol)
    entry = _conservative_zone_entry(direction, zone_low, zone_high)
    default_target_rr = _intraday_default_target_rr(atr_pct, symbol)
    min_liquidity_rr = _intraday_min_liquidity_rr(atr_pct, symbol)
    buy_side, sell_side = nearest_liquidity_targets(candles, price)
    swings = swing_points(candles, left=2, right=2)
    if direction == "long":
        stop = _bounded_structural_stop(direction, candles, swings, entry, zone_low, zone_high, local_atr, atr_pct, symbol)
    ...
    take_profits = _build_take_profits(direction, entry, stop, fallback_target, liquidity_target, target_levels, local_atr, symbol)
...
def _build_take_profits(
    ...
    tp2_max, tp3_max = _take_profit_rr_bounds(symbol)
    if direction == "long":
        risk = max(entry - stop, abs(entry) * 0.001)
        tp1 = _select_take_profit(direction, entry, risk, target_levels or [], 0.85, 1.35, 1.0, local_atr)
        tp2 = _select_take_profit(direction, entry, risk, target_levels or [], 1.45, tp2_max, _level_rr(direction, entry, risk, target), local_atr)
        if tp2 <= tp1:
        ...
        tp3 = _select_take_profit(
            ...
            risk,
            target_levels or [],
            max(2.1, _level_rr(direction, entry, risk, tp2) + 0.45),
            tp3_max,
...
def _evaluate_direction(
    ...
    if candles_15m or candles_1h:
        entry_zone, stop, target, rr, take_profits = _risk_setup(direction, candles_15m, candles_1h, price, fvg_zone, ote, ticker.symbol)
...
def _evaluate_direction_v2(
    ...
    if candles_15m or candles_1h:
        ...
        if entry_source and not fvg_zone and ote:
            entry_origin = "ote"
            entry_validity = "valid"
        entry_zone, stop, target, rr, take_profits = _risk_setup(direction, candles_15m, candles_1h, price, entry_source, ote, ticker.symbol)
        score.entry_zone = entry_zone
        score.stop = stop
        score.target = target
        score.take_profits = take_profits
