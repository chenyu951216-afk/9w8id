...
class DirectionScore:
    ...
    bonus_max_score: float = 0.0
    reasons: list[str] = field(default_factory=list)
    warnings: list[str] = field(default_factory=list)
    signal_notes: list[str] = field(default_factory=list)
    inactive_features: dict[str, str] = field(default_factory=dict)
    ...
    calibrated_available_max: float = 0.0
    ...
    def normalized(self) -> float:
        if self.calibrated_score is not None:
            return max(0.0, min(100.0, self.calibrated_score))
        if self.max_score <= 0:
            return 0.0
    ...
    def data_completeness(self) -> float:
        if self.calibrated_available_max > 0:
            return max(0.0, min(100.0, self.calibrated_available_max))
