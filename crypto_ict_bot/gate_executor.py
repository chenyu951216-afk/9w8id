...
def build_gate_execution(
    ...
    if _record_strategy_plan(trade_state, trade_mode, latest_price=report.price):
        _save_gate_state(state_path, trade_state)
    order_mode = _gate_order_mode(trade_state, trade_mode)
    intent = _build_order_intent(report, order_mode, settings)
    entry_guard = _entry_guard(settings, trade_state, intent)
    if entry_guard.get("state_changed"):
        _save_gate_state(state_path, trade_state)
    live_blockers = _live_blockers(settings, intent, order_mode) + entry_guard["blockers"]
    base_result = {
        "version": GATE_EXECUTOR_VERSION,
        ...
        "mode_state": order_mode.get("state", "missing_eth_plan"),
        "strategy_mode_state": trade_mode.get("state", "missing_eth_plan"),
        "order_intent": intent,
    ...
    if live_blockers:
        return {**base_result, "action": "blocked", "message": f"Gate live 下單被安全條件擋下：{_blocker_summary(live_blockers)}"}
...
def _settings(config: dict[str, Any]) -> dict[str, Any]:
    ...
    settings = {
        ...
        "margin_mode": str(gate.get("margin_mode") or "cross").lower(),
        ...
        "place_exit_orders": bool(gate.get("place_exit_orders", True)),
    ...
    if settings["margin_mode"] not in {"cross", "isolated"}:
        settings["margin_mode"] = "cross"
    if settings["enabled"] and not settings["dry_run"]:
        settings["margin_mode"] = "cross"
        settings["set_leverage_before_order"] = True
        settings["place_exit_orders"] = True
        settings["position_management_enabled"] = True
    return settings
...
def _gate_order_mode(state: dict[str, Any], trade_mode: dict[str, Any]) -> dict[str, Any]:
    """Recover a locked executable Gate plan when ETH strategy state is stale."""
    if str(trade_mode.get("state") or "") != "manage_existing":
        return trade_mode
    ...
    if pending or active:
        return trade_mode
    locked = state.get("strategy_plan") if isinstance(state.get("strategy_plan"), dict) else {}
    if str(locked.get("state") or "") != "execute_ready":
        return _recover_manage_existing_mode(trade_mode)
    mode = _mode_from_strategy_plan(locked)
    mode["state"] = "execute_ready"
    mode["decision"] = "Recovered locked Gate strategy plan; no real Gate pending order or position exists."
    mode["recovered_from_locked_strategy_plan"] = True
    return mode


def _recover_manage_existing_mode(trade_mode: dict[str, Any]) -> dict[str, Any]:
    direction = str(trade_mode.get("direction") or "")
    entry_zone = trade_mode.get("entry_zone") if isinstance(trade_mode.get("entry_zone"), (list, tuple)) else None
    stop = _as_float(trade_mode.get("stop"))
    if direction not in {"long", "short"} or not entry_zone or len(entry_zone) < 2 or stop is None:
        return trade_mode
    mode = dict(trade_mode)
    mode["state"] = "execute_ready"
    mode["decision"] = "Recovered ETH strategy active plan; Gate has no pending entry or live position."
    mode["recovered_from_strategy_active_plan"] = True
    return mode


def _mode_from_strategy_plan(plan: dict[str, Any]) -> dict[str, Any]:
    return {
        "mode": plan.get("mode") or "trader",
        "source_mode": plan.get("source_mode") or plan.get("mode") or "trader",
        "state": plan.get("state") or "observe",
        "direction": plan.get("direction"),
        "entry_zone": plan.get("entry_zone"),
        "entry_basis_for_rr": plan.get("entry_basis_for_rr"),
        "stop": plan.get("stop"),
        "take_profits": plan.get("take_profits") or [],
        "rr": plan.get("rr"),
        "quality_score": plan.get("quality_score"),
        "min_score": plan.get("min_score"),
        "execution_band_pct": plan.get("execution_band_pct"),
        "plan_id": plan.get("plan_id"),
        "decision": plan.get("decision"),
        "hard_blockers": plan.get("hard_blockers") or [],
        "soft_notes": plan.get("soft_notes") or [],
    }


def _record_strategy_plan(state: dict[str, Any], mode: dict[str, Any], latest_price: float | None = None) -> bool:
    old = state.get("strategy_plan") if isinstance(state.get("strategy_plan"), dict) else {}
    old_state = _strategy_plan_state(old, latest_price)
    if not isinstance(mode, dict):
        if old and old_state["valid"]:
            _annotate_held_strategy_plan(old, mode, old_state, "scan missing trader mode; keep locked plan until price invalidates it")
            state["strategy_plan"] = old
            return True
        if state.pop("strategy_plan", None) is not None:
            state["stale_strategy_plan"] = {**old, "cleared_reason": old_state["reason"]}
            return True
        return False
    mode_state = str(mode.get("state") or "")
    if mode_state not in {"execute_ready", "armed_wait_entry", "wait_retest"}:
        if old and old_state["valid"] and _can_hold_plan_through_scan(old, mode):
            _annotate_held_strategy_plan(old, mode, old_state, "current scan is observe, but locked trader plan is still alive")
            state["strategy_plan"] = old
            return True
        if state.pop("strategy_plan", None) is not None:
            state["stale_strategy_plan"] = {**old, "cleared_reason": old_state["reason"] if not old_state["valid"] else "current scan invalidated locked trader plan"}
            return True
        return False
    ...
    if direction not in {"long", "short"} or not entry_zone or len(entry_zone) < 2 or stop is None:
        if old and old_state["valid"]:
            _annotate_held_strategy_plan(old, mode, old_state, "new scan has incomplete plan; keep locked trader plan")
            state["strategy_plan"] = old
            return True
        if state.pop("strategy_plan", None) is not None:
            state["stale_strategy_plan"] = {**old, "cleared_reason": "new scan produced incomplete trader plan"}
            return True
        return False
    plan = {
    ...
    elif old and old_state["valid"] and _can_hold_plan_through_scan(old, plan):
        _annotate_held_strategy_plan(old, plan, old_state, "new scan rebuilt levels, but locked trader plan remains valid")
        old["latest_rebuilt_plan"] = {
            "plan_id": plan.get("plan_id"),
...
def _strategy_plan_state(plan: dict[str, Any], latest_price: float | None = None) -> dict[str, Any]:
    if not plan:
        return {"valid": False, "reason": "no locked trader plan"}
    now = time.time()
    ...
    if now - created_ts > max_age_seconds:
        return {"valid": False, "reason": "locked trader plan expired after 90 minutes", "age_seconds": round(now - created_ts, 1)}
    direction = str(plan.get("direction") or "")
    stop = _as_float(plan.get("stop"))
    price = _as_float(latest_price)
    entry_zone = plan.get("entry_zone") if isinstance(plan.get("entry_zone"), (list, tuple)) else None
    if price is not None and entry_zone and len(entry_zone) >= 2:
        low, high = sorted((float(entry_zone[0]), float(entry_zone[1])))
        distance_pct = 0.0 if low <= price <= high else min(abs(price - low), abs(price - high)) / max(abs(price), 1e-12) * 100.0
        max_distance_pct = max(3.0, min(8.0, (_as_float(plan.get("execution_band_pct")) or 0.6) * 8.0))
        if distance_pct > max_distance_pct:
            return {
                "valid": False,
                "reason": "locked trader plan is too far from current ETH price",
                "price": price,
                "entry_zone": [low, high],
                "distance_pct": round(distance_pct, 4),
                "max_distance_pct": round(max_distance_pct, 4),
            }
    if price is not None and stop is not None:
        if direction == "long" and price <= stop:
            return {"valid": False, "reason": "price invalidated locked long plan by touching stop", "price": price, "stop": stop}
        if direction == "short" and price >= stop:
            return {"valid": False, "reason": "price invalidated locked short plan by touching stop", "price": price, "stop": stop}
    return {"valid": True, "reason": "locked trader plan is still valid", "age_seconds": round(now - created_ts, 1)}


def _can_hold_plan_through_scan(old: dict[str, Any], new: dict[str, Any]) -> bool:
    ...
    if new_direction in {"long", "short"} and new_direction != old_direction:
        new_quality = _as_float(new.get("quality_score")) or 0.0
        new_min = _as_float(new.get("min_score")) or 74.0
        return new_quality < new_min + 6.0
    old_quality = _as_float(old.get("quality_score")) or 0.0
    new_quality = _as_float(new.get("quality_score")) or old_quality
    return max(old_quality, new_quality) >= 60.0


def _annotate_held_strategy_plan(plan: dict[str, Any], scan_mode: dict[str, Any], plan_state: dict[str, Any], reason: str) -> None:
    plan["status"] = "watching_price"
    plan["hold_reason"] = reason
    plan["validity"] = plan_state
    plan["last_scan_state"] = scan_mode.get("state") if isinstance(scan_mode, dict) else None
    plan["last_scan_direction"] = scan_mode.get("direction") if isinstance(scan_mode, dict) else None
    plan["last_scan_quality_score"] = _as_float(scan_mode.get("quality_score")) if isinstance(scan_mode, dict) else None
    plan["last_scan_decision"] = scan_mode.get("decision") if isinstance(scan_mode, dict) else None
    plan["last_scan_at"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
...
def _build_order_intent(report: SymbolReport, mode: dict[str, Any], settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if settings.get("margin_mode") == "cross":
        request_body["pos_margin_mode"] = "cross"
    return {
...
def _live_blockers(settings: dict[str, Any], intent: dict[str, Any], mode: dict[str, Any]) -> list[str]:
    ...
    if settings.get("margin_mode") != "cross":
        blockers.append("Gate live ETH entries must use cross margin mode.")
    if not settings.get("set_leverage_before_order", True):
        blockers.append("GATE_SET_LEVERAGE_BEFORE_ORDER must stay true so the bot can force cross margin before entry.")
    if mode.get("state") == "manage_existing":
        blockers.append("已有同方向計畫，不能重複開新倉。")
    return blockers


def _blocker_summary(blockers: list[Any], limit: int = 3) -> str:
    clean: list[str] = []
    for blocker in blockers:
        ...
        if len(clean) >= limit:
            break
    return "；".join(clean) if clean else "未提供具體 blocker"
...
def _market_close_order_body(
    ...
    if settings.get("margin_mode") == "cross":
        body["pos_margin_mode"] = "cross"
    return body


def _submit_reduce_only_market_close(
    settings: dict[str, Any],
    direction: str,
    contracts_abs: int,
    text: str,
) -> dict[str, Any]:
    body = _market_close_order_body(settings, direction, contracts_abs, text)
    return {"request": body, "response": _submit_order(settings, body)}


def _take_profit_specs(direction: str, contracts_abs: int, take_profits: list[dict[str, Any]]) -> list[dict[str, Any]]:
    clean_take_profits = [tp for tp in take_profits if isinstance(tp, dict)]
    contract_slices = _contract_slices(
        contracts_abs,
        [max(0.0, min(100.0, _as_float(tp.get("portion_pct")) or 0.0)) for tp in clean_take_profits],
    )
    specs: list[dict[str, Any]] = []
    for idx, (take_profit, close_contracts) in enumerate(zip(clean_take_profits, contract_slices), start=1):
        price = _as_float(take_profit.get("price"))
        if price is None or close_contracts <= 0:
            continue
        specs.append(
            {
                "index": idx,
                "text": f"t-eth-tp{idx}",
                "name": take_profit.get("name") or f"TP{idx}",
                "price": price,
                "contracts": int(close_contracts),
                "close_size": (-int(close_contracts) if direction == "long" else int(close_contracts)),
                "rule": _tp_rule(direction),
            }
        )
    return specs
...
def _submit_take_profit_specs(settings: dict[str, Any], specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    responses: list[dict[str, Any]] = []
    for spec in specs:
        body = _price_order_body(
            settings,
            close_size=int(spec["close_size"]),
            trigger_price=float(spec["price"]),
            rule=int(spec["rule"]),
            text=str(spec["text"]),
        )
        responses.append({"type": spec["name"], "request": body, "response": _submit_price_order(settings, body)})
    return responses


def _planned_take_profit_specs(specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "text": spec["text"],
            "close_size": spec["close_size"],
...
def _tp_reached(direction: str, mark_price: float, take_profit_price: float) -> bool:
    return mark_price >= take_profit_price if direction == "long" else mark_price <= take_profit_price


def _stop_reached(direction: str, mark_price: float, stop_price: float) -> bool:
    return mark_price <= stop_price if direction == "long" else mark_price >= stop_price


def _matching_price_orders(bot_orders: list[dict[str, Any]], text: str) -> list[dict[str, Any]]:
    return [order for order in bot_orders if order.get("text") == text]


def _sync_executed_take_profits(active: dict[str, Any], specs: list[dict[str, Any]], current_contracts_abs: int) -> set[str]:
    executed = {str(text) for text in (active.get("executed_take_profits") or []) if text}
    initial_contracts = int(_as_float(active.get("initial_contracts_abs")) or current_contracts_abs)
    closed_contracts = max(0, initial_contracts - current_contracts_abs)
    cumulative = 0
    for spec in specs:
        cumulative += int(spec["contracts"])
        if closed_contracts >= cumulative:
            executed.add(str(spec["text"]))
    active["executed_take_profits"] = sorted(executed)
    return executed


def _close_action_recent(active: dict[str, Any], key: str) -> bool:
    submitted_ts = _as_float(active.get(key))
    return submitted_ts is not None and time.time() - submitted_ts < GATE_CLOSE_RETRY_SECONDS


def _take_profit_market_close_action(
    settings: dict[str, Any],
    direction: str,
    spec: dict[str, Any],
    close_contracts: int,
    ...
    text = str(spec["text"])
    action = {
        "type": "take_profit_market_close",
        "text": text,
        "target_price": round(float(spec["price"]), 2),
        "mark_price": round(float(mark_price), 2),
        "contracts": int(close_contracts),
        "mode": "live" if live_ready else "dry_run",
    }
    matching_orders = _matching_price_orders(bot_orders, text)
    if live_ready:
        if matching_orders:
            cancel_actions = _cancel_bot_orders(settings, matching_orders, live_ready)
            action["cancelled"] = cancel_actions
            if any(cancel.get("error") for cancel in cancel_actions):
                action["error"] = "matching TP price order cancel failed; skip market close to avoid duplicate close."
                return action
        try:
            action.update(_submit_reduce_only_market_close(settings, direction, int(close_contracts), f"{text}-close"))
        except Exception as exc:
            action["error"] = f"{type(exc).__name__}: {exc}"
    else:
        action["planned_order"] = _market_close_order_body(settings, direction, int(close_contracts), f"{text}-close")
    return action


def _stop_market_close_action(
    settings: dict[str, Any],
    direction: str,
    contracts_abs: int,
    stop_price: float,
    ...
    action = {
        "type": "stop_market_close",
        "stop_price": round(float(stop_price), 2),
        "mark_price": round(float(mark_price), 2),
        "contracts": int(contracts_abs),
        "mode": "live" if live_ready else "dry_run",
    }
    if live_ready:
        if bot_orders:
            cancel_actions = _cancel_bot_orders(settings, bot_orders, live_ready)
            action["cancelled"] = cancel_actions
            if any(cancel.get("error") for cancel in cancel_actions):
                action["error"] = "bot price order cancel failed; skip emergency close to avoid duplicate close."
                return action
        try:
            action.update(_submit_reduce_only_market_close(settings, direction, int(contracts_abs), "t-eth-stop-close"))
        except Exception as exc:
            action["error"] = f"{type(exc).__name__}: {exc}"
    else:
        action["planned_order"] = _market_close_order_body(settings, direction, int(contracts_abs), "t-eth-stop-close")
    return action


def _close_action_submitted(action: dict[str, Any]) -> bool:
    return "response" in action and not action.get("error")
...
def _manage_open_position(
    ...
    initial_contracts_abs = int(_as_float(active.get("initial_contracts_abs")) or contracts_abs)
    tp_specs = _take_profit_specs(direction, initial_contracts_abs, plan.get("take_profits", []))
    executed_take_profits = _sync_executed_take_profits(active, tp_specs, contracts_abs)

    if settings["place_exit_orders"] and _stop_reached(direction, mark_price, desired_stop):
        if _close_action_recent(active, "stop_market_close_submitted_ts"):
            actions.append(
                {
                    "type": "stop_market_close_pending",
            ...
            action = _stop_market_close_action(settings, direction, contracts_abs, desired_stop, bot_orders, live_ready, mark_price)
            actions.append(action)
            if _close_action_submitted(action):
                active["stop_market_close_submitted_ts"] = time.time()
                active["stop_market_close_submitted_at"] = now_text
        active["last_stop"] = round(desired_stop, 8)
        state["active_position"] = active
        return actions

    remaining_contracts = contracts_abs
    for spec in tp_specs:
        text = str(spec["text"])
        if text in executed_take_profits or not _tp_reached(direction, mark_price, float(spec["price"])):
            continue
        if _close_action_recent(active, f"{text}_market_close_submitted_ts"):
            actions.append(
                {
                    "type": "take_profit_market_close_pending",
                    "text": text,
                    "target_price": round(float(spec["price"]), 2),
                    "mark_price": round(mark_price, 2),
                    "mode": "live" if live_ready else "dry_run",
            ...
            continue
        close_contracts = min(int(spec["contracts"]), remaining_contracts)
        if close_contracts <= 0:
            continue
        action = _take_profit_market_close_action(settings, direction, spec, close_contracts, bot_orders, live_ready, mark_price)
        actions.append(action)
        if _close_action_submitted(action):
            executed_take_profits.add(text)
            active["executed_take_profits"] = sorted(executed_take_profits)
            active[f"{text}_market_close_submitted_ts"] = time.time()
            active[f"{text}_market_close_submitted_at"] = now_text
            remaining_contracts = max(0, remaining_contracts - close_contracts)

    tp_order_texts = {str(order.get("text") or "") for order in _tp_orders(bot_orders)}
    missing_tp_specs = [
        spec
        for spec in tp_specs
        if spec["text"] not in tp_order_texts
        and spec["text"] not in executed_take_profits
        and not _tp_reached(direction, mark_price, float(spec["price"]))
    ]
    if settings["place_exit_orders"] and missing_tp_specs:
        action = {"type": "place_missing_take_profits", "mode": "live" if live_ready else "dry_run"}
        if live_ready:
            try:
                action["responses"] = _submit_take_profit_specs(settings, missing_tp_specs)
            except Exception as exc:
                action["error"] = f"{type(exc).__name__}: {exc}"
        else:
            action["planned_orders"] = _planned_take_profit_specs(missing_tp_specs)
        actions.append(action)
...
def _refresh_active_position_state(
    ...
    if same_position:
        ...
        if not active.get("initial_contracts_abs"):
            active["initial_contracts_abs"] = contracts_abs
        if plan.get("take_profits"):
            active["take_profits"] = plan.get("take_profits", [])
    ...
    return {
        ...
        "initial_contracts_abs": contracts_abs,
        "initial_stop": float(plan["stop"]),
...
def _live_ready(settings: dict[str, Any]) -> bool:
    return (
        ...
        and settings["place_exit_orders"]
    )
...
def _evaluate_strategy_plan_trigger(plan: dict[str, Any], ticker: dict[str, Any]) -> dict[str, Any]:
    ...
    min_trigger_score = max(64.0, min_score - 10.0)
...
def _set_leverage(settings: dict[str, Any]) -> Any:
    if settings.get("margin_mode") == "cross":
        cross_mode_response = _gate_request(
            settings,
            "POST",
            f"/futures/{settings['settle']}/positions/cross_mode",
            body={"contract": settings["contract"], "mode": "cross"},
        )
        leverage_response = _gate_request(
            settings,
            "POST",
            f"/futures/{settings['settle']}/positions/{settings['contract']}/leverage",
            body=None,
            query_params={"leverage": "0", "cross_leverage_limit": str(settings["leverage"])},
        )
        return {"margin_mode_response": cross_mode_response, "leverage_response": leverage_response}
...
def check_gate_connection(config: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "place_exit_orders": settings["place_exit_orders"],
        "confirm_live_trading_ok": settings["confirm_live_trading"] == GATE_LIVE_CONFIRMATION,
        "account_ok": False,
        "account": {},
        "errors": [],
        "safety_blockers": [],
    ...
    result["safety_blockers"] = _connection_safety_blockers(settings, contract)
    if not settings["api_key"] or not settings["api_secret"]:
        result["errors"].append("缺少 Gate API key/secret，無法測試帳戶。")
        result["errors"].extend(result["safety_blockers"])
        return result
    try:
    ...
    result["live_ready"] = (
        ...
        and not settings["dry_run"]
        and not result["safety_blockers"]
    )
    return result


def _connection_safety_blockers(settings: dict[str, Any], contract: dict[str, Any]) -> list[str]:
    blockers: list[str] = []
    if not settings["enabled"]:
        blockers.append("GATE_TRADING_ENABLED is not true.")
    if settings["dry_run"]:
        blockers.append("GATE_TRADING_DRY_RUN is still true.")
    if settings["confirm_live_trading"] != GATE_LIVE_CONFIRMATION:
        blockers.append("GATE_CONFIRM_LIVE_TRADING is missing or incorrect.")
    if not settings["api_key"] or not settings["api_secret"]:
        blockers.append("Gate API key/secret is missing.")
    if not bool(contract.get("contract_size_eth") or settings["contract_size_eth"] > 0):
        blockers.append("Gate ETH contract multiplier is missing; size cannot be calculated.")
    if not settings["place_exit_orders"]:
        blockers.append("GATE_PLACE_EXIT_ORDERS must be true before live ETH entries.")
    if settings.get("margin_mode") != "cross":
        blockers.append("Gate live ETH entries must use cross margin mode.")
    if not settings.get("set_leverage_before_order", True):
        blockers.append("GATE_SET_LEVERAGE_BEFORE_ORDER must stay true so the bot can force cross margin before entry.")
    if float(settings["margin_usdt"]) > 15.0:
        blockers.append("GATE_MARGIN_USDT is above the 15U safety cap.")
    if int(settings["leverage"]) > 200:
        blockers.append("GATE_LEVERAGE is above the 200x safety cap.")
...
def manage_gate_position(
                ...
                result["planned_actions"].append(
                    {
                        ...
                        "message": "ETH 價格已回到 trader plan 入場區；但缺少 Gate API，無法送實單。",
                        "mode": "blocked",
                        "trigger": trigger,
                    }
                )
    ...
    if position_payload.get("position_ok") is False:
        result["planned_actions"].append(
            {
                "type": "position_poll_failed",
                "message": "Gate position poll failed; keeping local position state until the next successful poll.",
            }
        )
        result["state"] = _compact_gate_state(state)
